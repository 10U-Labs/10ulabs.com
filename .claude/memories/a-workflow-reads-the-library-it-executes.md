...
`bootstrap.yml` is the case. It deploys Terraform and no Lambda source of its own, and it carries a `pylint-source`, a `mypy-source` and a `copy-paste-source` job whose whole argument list is `lib/python/` packages — ten of the twelve, being the ten its own suites import. `www_home.yml` and `www_rack_designer.yml` are in the same position. The alternative, folding the library into the test passes of those workflows, would lint library source under `--recursive=y` with `scripts/` on the import path and would report a library defect under a step named "Run pylint on tests", which is the one thing the four-pass split exists to prevent.
