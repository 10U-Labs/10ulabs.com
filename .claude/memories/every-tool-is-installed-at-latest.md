...
Eighteen packages come from `python3 -m pip install`: `assert-no-comments`, `assert-no-inline-directives`, `assert-no-linter-config-files`, `assert-no-pytest-plugin-declarations`, `assert-one-assert-per-pytest`, `assert-pytest-fixture-name-is-needed`, `boto3`, `boto3-stubs`, `botocore`, `dnspython`, `mypy`, `pylint`, `pytest`, `pytest-cov`, `python-hcl2`, `requests`, `types-requests`, `yamllint`. Three come from `npm install -g`: `jscpd`, `jsonlint`, `markdownlint-cli`. Three more come from the `npm install --no-save` in `www_rack_designer.yml`: `eslint`, `@eslint/js`, `globals`. Two more tools come from neither, being GitHub Actions rather than packages: `10U-Labs/assert-opentofu-resource-is-used@latest` and `10U-Labs/assert-python-definition-is-used@latest`. Read the current set from the files rather than from this list, which is a snapshot: `grep -rn 'pip install' -A 20 .github/workflows/`, `grep -rn 'npm install' .github/workflows/` and `grep -rn '10U-Labs/' .github/workflows/`.

## Why a pin is worse than the surprise it prevents

A pinned checker is a checker that has stopped checking. The eight `10U-Labs` tools are written to be extended — a new suffix, a new shape refused, a defect fixed in a reader — and the point of installing them here is that the day a release can see more, this repository is held to it. A pin defers every one of those findings to whenever somebody edits the pin, and nothing schedules that, so the pinned version is the one that was current the day the line was written and the findings accumulate unreported behind it. The same is true of `pylint`, `mypy` and `yamllint`: a release that adds a checker is a check this repository wants, and a bounded specifier like `<8` is a pin with a slower fuse rather than a different kind of thing.
