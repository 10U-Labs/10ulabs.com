...
Unit tests alone are not sufficient. Add coverage at every tier the change touches. One assert per pytest, which is not a style preference here: `Assert one assert per pytest` is a step in twenty-seven of the twenty-eight workflows — `rtl_verilator.yml` is the one without it — so a test with two asserts fails the push.
