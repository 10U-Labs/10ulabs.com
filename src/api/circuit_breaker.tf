resource "aws_sns_topic" "circuit_breaker_alerts" {
  name = "${var.resource_prefix}-circuit-breaker-alerts"
...
resource "aws_cloudwatch_metric_alarm" "circuit_breaker_open" {
  alarm_name          = "${var.resource_prefix}-circuit-breaker-open"
  ...
  tags = {
    Name = "${var.resource_prefix}-circuit-breaker-open"
  }
}

resource "aws_cloudwatch_metric_alarm" "circuit_breaker_high_failures" {
  alarm_name          = "${var.resource_prefix}-circuit-breaker-high-failures"
  ...
  tags = {
    Name = "${var.resource_prefix}-circuit-breaker-high-failures"
  }
}

resource "aws_cloudwatch_metric_alarm" "webhook_handler_errors" {
  alarm_name          = "${var.resource_prefix}-webhook-handler-errors"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 2
  threshold           = 5
  ...
  tags = {
    Name = "${var.resource_prefix}-webhook-handler-errors"
  }
}

resource "aws_cloudwatch_metric_alarm" "job_queue_dlq_messages" {
  alarm_name          = "${var.resource_prefix}-job-queue-dlq-messages"
  comparison_operator = "GreaterThanThreshold"
  ...
  tags = {
    Name = "${var.resource_prefix}-job-queue-dlq-messages"
  }
}

resource "aws_cloudwatch_event_rule" "circuit_breaker_remediation" {
  name        = "${var.resource_prefix}-circuit-breaker-remediation"
...
resource "aws_cloudwatch_event_rule" "dlq_reprocessor" {
  name                = "${var.resource_prefix}-dlq-reprocessor"
...
resource "aws_cloudwatch_event_rule" "circuit_breaker_recovery" {
  name                = "${var.resource_prefix}-circuit-breaker-recovery"
