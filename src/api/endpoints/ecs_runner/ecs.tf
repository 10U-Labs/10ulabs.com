  ...
  container_definitions = jsonencode([
    {
      ...
      # Volume permissions are fixed by Lambda command override (sudo chown)
      mountPoints = [
        {
    ...
    },
    {
      ...
      # Write config to expected location then start agent
      command = [
        "/bin/bash", "-c",
        "echo \"$CW_CONFIG_CONTENT\" > /opt/aws/amazon-cloudwatch-agent/etc/amazon-cloudwatch-agent.json && exec /opt/aws/amazon-cloudwatch-agent/bin/start-amazon-cloudwatch-agent"
      ]
