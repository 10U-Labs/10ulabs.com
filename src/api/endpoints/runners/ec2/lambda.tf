data "archive_file" "handler" {
  ...
  source {
    content  = file("${path.module}/lambda/fleet_ops.py")
    filename = "fleet_ops.py"
  }
  source {
...
resource "aws_lambda_function" "handler" {
  ...
  timeout          = 60
  memory_size      = 256
  description      = "Handler for /v1/runners/ec2 API endpoints"
...
# Note: SQS event source mapping removed - /v1/runners now routes via HTTP to /v1/runners/ec2
