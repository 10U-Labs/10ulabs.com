...
def trigger_ecs_image_build(_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result = trigger_github_workflow('api_endpoint_v1_runners_ecs_images_post.yml', payload)
    return result
...
ROUTE_MAP = {
    ('/v1/runners/ecs/images', 'POST'): lambda e: handle_post_request(
        e, trigger_ecs_image_build
    ),
    ('/v1/runners/ecs/images', 'GET'): handle_ecs_image_get,
    ('/v1/runners/ecs/images/latest', 'GET'): handle_ecs_image_get,
}
...
def lambda_handler(event, _context):
    ...
    if not handler:
        if path.startswith('/v1/runners/ecs/images/') and not path.endswith('/latest'):
            if method == 'GET':
