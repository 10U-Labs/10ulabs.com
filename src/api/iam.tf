...
resource "aws_iam_role" "scheduled_termination" {
  name = "${local.resource_prefix}-ScheduledTermination-Role"
...
resource "aws_iam_role_policy_attachment" "scheduled_termination_basic" {
  role       = aws_iam_role.scheduled_termination.name
  policy_arn = "arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole"
}

resource "aws_iam_role_policy" "scheduled_termination_permissions" {
  name = "ScheduledTerminationPermissions"
  role = aws_iam_role.scheduled_termination.id
...
resource "aws_iam_role" "scheduler_execution" {
  name = "${local.resource_prefix}-SchedulerExecution-Role"
    ...
    Statement = [
      {
        ...
        Resource = aws_lambda_function.scheduled_termination.arn
      }
    ]
  })
}
