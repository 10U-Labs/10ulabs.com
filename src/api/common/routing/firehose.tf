...
# Firehose in us-east-1 for WAF logs (CloudFront WAF must be in us-east-1)
# Writes to the same central S3 bucket in us-east-2
resource "aws_kinesis_firehose_delivery_stream" "waf_logs" {
  ...
  extended_s3_configuration {
    ...
    prefix              = "cloudwatch-logs/waf/"
    error_output_prefix = "cloudwatch-logs/waf-errors/"
...
resource "aws_iam_role_policy" "firehose_waf_s3_access" {
  name = "S3Access"
...
resource "aws_iam_role" "cloudwatch_logs_firehose_waf" {
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        StringLike = {
          "aws:SourceArn" = "arn:aws:logs:us-east-1:${local.aws_account_id}:*"
        }
      }
    }]
  })
...
resource "aws_iam_role_policy" "cloudwatch_logs_firehose_waf_access" {
  name = "FirehoseAccess"
  role = aws_iam_role.cloudwatch_logs_firehose_waf.id
