...
data "archive_file" "scheduled_termination" {
  type        = "zip"
  source_file = "${path.module}/lambdas/scheduled_termination.py"
  output_path = "${path.module}/.terraform/lambda_packages/scheduled_termination.zip"
}

resource "aws_lambda_function" "scheduled_termination" {
  filename         = data.archive_file.scheduled_termination.output_path
  function_name    = "${local.resource_prefix}-ScheduledTermination"
  role             = aws_iam_role.scheduled_termination.arn
  handler          = "scheduled_termination.lambda_handler"
  source_code_hash = data.archive_file.scheduled_termination.output_base64sha256
  runtime          = "python3.13"
  timeout          = 60
  memory_size      = 128
  description      = "Terminates runners on scheduled events"
  ...
  logging_config {
    log_format = "Text"
    log_group  = aws_cloudwatch_log_group.scheduled_termination.name
  }

  tags = merge(local.common_tags, {
    Name = "${local.resource_prefix}-ScheduledTermination"
  })
}

resource "aws_cloudwatch_log_group" "scheduled_termination" {
  name              = "/aws/lambda/${local.resource_prefix}-ScheduledTermination"
  retention_in_days = 7

  tags = merge(local.common_tags, {
    Name = "${local.resource_prefix}-ScheduledTermination-logs"
  })
}

resource "aws_lambda_permission" "scheduled_termination_from_scheduler" {
  statement_id  = "AllowSchedulerInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.scheduled_termination.function_name
