# AgentCore Runtime for Workflow Fixer Agent
resource "aws_bedrockagentcore_agent_runtime" "workflow_fixer" {
  agent_runtime_name = replace(local.stack_name, "-", "_")
  description        = "Workflow Fixer Agent - analyzes failures and creates fix PRs"
