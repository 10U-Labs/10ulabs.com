...
def test_fargate_runner_task_appears_in_cluster(api_url, api_key, github_repo, ecs_client, tfvars):
    headers = {"x-api-key": api_key}
...
def test_fargate_runner_cluster_exists(ecs_client, tfvars):
    cluster_name = tfvars["cluster_name"]
    response = ecs_client.describe_clusters(clusters=[cluster_name])
    assert len(response['clusters']) == 1


def test_fargate_runner_cluster_is_active(ecs_client, tfvars):
    cluster_name = tfvars["cluster_name"]
    response = ecs_client.describe_clusters(clusters=[cluster_name])
