...
def test_sns_circuit_breaker_alerts_has_email_subscription(sns_client, tfvars):
    topic_name = f"{tfvars['resource_prefix']}-circuit-breaker-alerts"
...
def test_sns_subscription_protocol_is_email(sns_client, tfvars):
    topic_name = f"{tfvars['resource_prefix']}-circuit-breaker-alerts"
    topics = sns_client.list_topics()
    topic_arn = next((t['TopicArn'] for t in topics['Topics'] if topic_name in t['TopicArn']), None)
...
def test_sns_subscription_endpoint_matches_tfvars(sns_client, tfvars):
    topic_name = f"{tfvars['resource_prefix']}-circuit-breaker-alerts"
    topics = sns_client.list_topics()
    topic_arn = next((t['TopicArn'] for t in topics['Topics'] if topic_name in t['TopicArn']), None)
    subscriptions = sns_client.list_subscriptions_by_topic(TopicArn=topic_arn)
    email_subs = [s for s in subscriptions['Subscriptions'] if s['Protocol'] == 'email']
    assert email_subs[0]['Endpoint'] == tfvars['circuit_breaker_alert_email']


def test_sns_topic_has_display_name(sns_client, tfvars):
    topic_name = f"{tfvars['resource_prefix']}-circuit-breaker-alerts"
    topics = sns_client.list_topics()
    topic_arn = next((t['TopicArn'] for t in topics['Topics'] if topic_name in t['TopicArn']), None)
...
def test_sns_topic_arn_format(sns_client, tfvars):
    topic_name = f"{tfvars['resource_prefix']}-circuit-breaker-alerts"
    topics = sns_client.list_topics()
    topic_arn = next((t['TopicArn'] for t in topics['Topics'] if topic_name in t['TopicArn']), None)
