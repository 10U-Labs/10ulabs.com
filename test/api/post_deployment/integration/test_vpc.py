def test_vpc_cidr_block_configuration(ec2_client, config):
    vpcs = ec2_client.describe_vpcs(Filters=[{'Name': 'tag:Name', 'Values': [config["vpc_name"]]}])
    assert len(vpcs['Vpcs'][0]['CidrBlock']) > 0


def test_public_subnets_have_internet_gateway_route(ec2_client, config):
    vpcs = ec2_client.describe_vpcs(Filters=[{'Name': 'tag:Name', 'Values': [config["vpc_name"]]}])
...
def test_subnets_span_multiple_availability_zones(ec2_client, config):
    vpcs = ec2_client.describe_vpcs(Filters=[{'Name': 'tag:Name', 'Values': [config["vpc_name"]]}])
    vpc_id = vpcs['Vpcs'][0]['VpcId']
...
def test_nat_gateway_configuration(ec2_client, config):
    vpcs = ec2_client.describe_vpcs(Filters=[{'Name': 'tag:Name', 'Values': [config["vpc_name"]]}])
    vpc_id = vpcs['Vpcs'][0]['VpcId']
    subnets = ec2_client.describe_subnets(Filters=[{'Name': 'vpc-id', 'Values': [vpc_id]}])
...
def test_security_group_ingress_rules(ec2_client, config):
    vpcs = ec2_client.describe_vpcs(Filters=[{'Name': 'tag:Name', 'Values': [config["vpc_name"]]}])
    vpc_id = vpcs['Vpcs'][0]['VpcId']
...
def test_security_group_egress_rules(ec2_client, config):
    vpcs = ec2_client.describe_vpcs(Filters=[{'Name': 'tag:Name', 'Values': [config["vpc_name"]]}])
    vpc_id = vpcs['Vpcs'][0]['VpcId']
    sgs = ec2_client.describe_security_groups(Filters=[{'Name': 'vpc-id', 'Values': [vpc_id]}])
