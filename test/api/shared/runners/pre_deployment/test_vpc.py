"""Pre-deployment tests for api/shared/runners VPC Terraform configuration."""
import re


def test_vpc_tf_defines_vpc(api_shared_runners_dir):
    """Test that vpc.tf defines the VPC."""
    vpc_tf = api_shared_runners_dir / "vpc.tf"
    content = vpc_tf.read_text()
    pattern = r'resource\s+"aws_vpc"\s+"main"'
    assert re.search(pattern, content) is not None


def test_vpc_has_correct_cidr(api_shared_runners_dir):
    """Test that VPC uses the expected CIDR block."""
    locals_tf = api_shared_runners_dir / "locals.tf"
    content = locals_tf.read_text()
    assert "10.0.0.0/16" in content


def test_vpc_has_dns_support(api_shared_runners_dir):
...
def test_vpc_has_dns_hostnames(api_shared_runners_dir):
...
def test_vpc_has_public_subnets(api_shared_runners_dir):
...
def test_vpc_has_internet_gateway(api_shared_runners_dir):
...
def test_security_group_defined(api_shared_runners_dir):
