...
def test_webhook_router_lambda_exists(lambda_client):
    functions = lambda_client.list_functions()
    function_names = [fn['FunctionName'] for fn in functions['Functions']]
    webhook_functions = [name for name in function_names if 'WebhookRouter' in name or 'webhook-router' in name.lower()]
    assert len(webhook_functions) > 0


def test_webhook_router_lambda_has_sqs_trigger(lambda_client):
    functions = lambda_client.list_functions()
    function_names = [fn['FunctionName'] for fn in functions['Functions']]
    webhook_functions = [name for name in function_names if 'WebhookRouter' in name or 'webhook-router' in name.lower()]
    if webhook_functions:
        event_sources = lambda_client.list_event_source_mappings(FunctionName=webhook_functions[0])
        assert 'EventSourceMappings' in event_sources


def test_circuit_breaker_lambda_exists(lambda_client):
    functions = lambda_client.list_functions()
    function_names = [fn['FunctionName'] for fn in functions['Functions']]
    cb_functions = [name for name in function_names if 'CircuitBreaker' in name or 'circuit-breaker' in name.lower()]
    assert len(cb_functions) > 0


def test_dlq_reprocessor_lambda_exists(lambda_client):
    functions = lambda_client.list_functions()
    function_names = [fn['FunctionName'] for fn in functions['Functions']]
    dlq_functions = [name for name in function_names if 'DLQReprocessor' in name or 'dlq-reprocessor' in name.lower()]
    assert len(dlq_functions) > 0


def test_configure_webhook_lambda_exists(lambda_client):
    functions = lambda_client.list_functions()
    function_names = [fn['FunctionName'] for fn in functions['Functions']]
    config_functions = [name for name in function_names if 'ConfigureWebhook' in name or 'configure-webhook' in name.lower()]
...
def test_job_queue_exists():
    ...
    queues = sqs.list_queues(QueueNamePrefix='runner')
...
def test_idempotency_table_exists():
    ...
    idempotency_tables = [t for t in tables['TableNames'] if 'idempotency' in t.lower() or 'webhook' in t.lower()]
    assert len(idempotency_tables) > 0


def test_idempotency_table_has_ttl():
    import boto3
    dynamodb = boto3.client('dynamodb')
    tables = dynamodb.list_tables()
    idempotency_tables = [t for t in tables['TableNames'] if 'idempotency' in t.lower() or 'webhook' in t.lower()]
    if idempotency_tables:
        ttl_status = dynamodb.describe_time_to_live(TableName=idempotency_tables[0])
        assert 'TimeToLiveDescription' in ttl_status


def test_webhook_dlq_exists():
    import boto3
    sqs = boto3.client('sqs')
    queues = sqs.list_queues(QueueNamePrefix='webhook')
    assert 'QueueUrls' in queues


def test_job_dlq_exists():
    import boto3
    sqs = boto3.client('sqs')
    queues = sqs.list_queues(QueueNamePrefix='job')
    dlq_queues = [q for q in queues.get('QueueUrls', []) if 'dlq' in q.lower() or 'dead' in q.lower()]
