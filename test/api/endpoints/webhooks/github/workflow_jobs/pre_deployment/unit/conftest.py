...
RUNNERS_SRC_PATH = (
    REPO_ROOT / "src" / "api" / "endpoints" / "webhooks" / "github" / "workflow_jobs"
)
RUNNERS_LAMBDAS_PATH = RUNNERS_SRC_PATH / "lambdas"

# Add lambdas path to sys.path so common module can be found when patching
if str(RUNNERS_LAMBDAS_PATH) not in sys.path:
    sys.path.insert(0, str(RUNNERS_LAMBDAS_PATH))
