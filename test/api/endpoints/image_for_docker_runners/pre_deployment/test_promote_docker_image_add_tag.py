...
def test_add_tag_returns_zero_on_success():
    mock_client = Mock()
    result = promote_docker_image.add_tag_to_image(mock_client, "repo", "{}", "tag")
    assert result == 0
...
def test_add_tag_passes_correct_image_tag():
    mock_client = Mock()
    promote_docker_image.add_tag_to_image(mock_client, "repo", "{}", "my-tag")
    assert mock_client.put_image.call_args[1]["imageTag"] == "my-tag"


def test_add_tag_passes_correct_manifest():
    mock_client = Mock()
    promote_docker_image.add_tag_to_image(mock_client, "repo", "{\"test\":true}", "tag")
