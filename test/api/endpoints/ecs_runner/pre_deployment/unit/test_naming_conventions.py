...
IAM_FILE = ECS_RUNNER_SRC / "iam.tf"
ECS_IAM_FILE = ECS_RUNNER_SRC / "ecs_iam.tf"
...
def extract_iam_role_names(tf_file: Path) -> list:
    ...
    for match in re.finditer(role_pattern, content):
        ...
        if name_match:
            ...
            resolved = resolved.replace("${var.task_family}", "fargate")
...
IAM_ROLES = (
    ...
    + extract_iam_role_names(ECS_IAM_FILE)
)
