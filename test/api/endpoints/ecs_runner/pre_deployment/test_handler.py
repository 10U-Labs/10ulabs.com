...
def test_lambda_handler_ecs_runner_post_with_missing_job_id_returns_400(ecs_runner_handler, ecs_runner_post_event_factory, lambda_context):
...
def test_lambda_handler_ecs_runner_post_with_missing_repo_returns_400(ecs_runner_handler, ecs_runner_post_event_factory, lambda_context):
    event = ecs_runner_post_event_factory()
    body = parse_response_body({'body': event['body']})
...
@patch('boto3.client')
def test_lambda_handler_ecs_runner_post_returns_json_content_type(mock_boto_client, ecs_runner_handler, ecs_runner_post_event_factory, lambda_context):
    mock_ecr = MagicMock()
...
@patch('boto3.client')
def test_lambda_handler_ecs_runner_does_not_specify_launch_type_and_capacity_provider(mock_boto_client, ecs_runner_handler, ecs_runner_post_event_factory, lambda_context):
    mock_ecr = MagicMock()
    mock_ecr.describe_images.return_value = {
    ...
    def mock_client(service_name):
        ...
        return MagicMock()
    mock_boto_client.side_effect = mock_client
    with patch.object(ecs_runner_handler, 'get_runner_registration_token', return_value='test-registration-token'):
...
@patch('boto3.client')
def test_launch_fargate_runner_tags_use_lowercase_key_for_type(mock_boto_client, ecs_runner_handler, ecs_runner_post_event_factory, lambda_context):
    mock_ecr = MagicMock()
    mock_ecr.describe_images.return_value = {
        'imageDetails': [{'imageTags': ['stable'], 'imagePushedAt': datetime(2024, 1, 1), 'imageDigest': 'sha256:test', 'imageSizeInBytes': 1000}]
    }
...
@patch('boto3.client')
def test_lambda_handler_ecs_runner_get_returns_json_content_type(mock_boto_client, ecs_runner_handler, lambda_context):
    mock_ecs = MagicMock()
...
def test_lambda_handler_ecs_runner_unsupported_method_returns_404(ecs_runner_handler, lambda_context):
...
def test_launch_fargate_runner_ecs_run_task_success(mock_boto_client, ecs_runner_handler):
    with patch.dict('os.environ', {'ECS_CLUSTER': 'test-cluster', 'TASK_DEFINITION': 'test-task', 'SUBNETS': 'subnet-1', 'SECURITY_GROUPS': 'sg-1', 'CONTAINER_NAME': 'test-container', 'GITHUB_TOKEN_SECRET_NAME': '/test/token'}):
        ...
        def mock_client(service):
            ...
            return MagicMock()
        mock_boto_client.side_effect = mock_client
        with patch.object(ecs_runner_handler, 'get_runner_registration_token', return_value='test-reg-token'):
...
def test_lambda_handler_options_request_returns_allow_origin_header(ecs_runner_handler, lambda_context):
    event = {'path': '/v1/ecs-runner', 'httpMethod': 'OPTIONS'}
    response = ecs_runner_handler.lambda_handler(event, lambda_context)
...
def test_lambda_handler_test_mode_returns_mock_for_ecs_runner_post(ecs_runner_handler, lambda_context):
    ecs_runner_handler.set_test_mode(False)
    event = {'path': '/v1/ecs-runner', 'httpMethod': 'POST', 'headers': {'x-test-mode': 'true'}, 'body': '{"job_id": 123, "github_repo": "test/repo"}'}
    response = ecs_runner_handler.lambda_handler(event, lambda_context)
...
def test_get_fargate_task_status_returns_status_from_describe_tasks(ecs_runner_handler):
    ...
    with patch.object(ecs_runner_handler, 'get_ecs_client', return_value=mock_ecs):
        result = ecs_runner_handler.get_fargate_task_status('test-cluster', 'arn:aws:ecs:us-east-1:123:task/test')
...
def test_wait_for_fargate_task_provisioned_returns_success_when_running(ecs_runner_handler):
    with patch.object(ecs_runner_handler, 'get_fargate_task_status', return_value={'status': 'RUNNING', 'stopped_reason': '', 'started_at': '2024-01-01'}):
        result = ecs_runner_handler.wait_for_fargate_task_provisioned('test-cluster', 'arn:aws:ecs:us-east-1:123:task/test')
        assert result['success'] is True
...
def test_wait_for_fargate_task_provisioned_detects_spot_interruption(ecs_runner_handler):
    with patch.object(ecs_runner_handler, 'get_fargate_task_status', return_value={'status': 'STOPPED', 'stopped_reason': 'Your Spot Task was interrupted.', 'started_at': None}):
        result = ecs_runner_handler.wait_for_fargate_task_provisioned('test-cluster', 'arn:aws:ecs:us-east-1:123:task/test')
        assert result['success'] is False
