...
def test_runner_creation_requires_auth(api_url):
    ...
    response_is_forbidden = response.status_code == 403
    assert response_is_forbidden
...
def test_ec2_runner_status_accepts_valid_api_key(api_url, api_key):
    ...
    response_is_successful = response.status_code == 200
    assert response_is_successful


def test_ec2_runner_status_returns_json(api_url, api_key):
    response = make_authenticated_get(f"{api_url}/v1/ec2-runner", api_key, timeout=10)
    content_type_is_json = response.headers["Content-Type"] == "application/json"
    assert content_type_is_json


def test_ec2_runner_status_has_success_field(api_url, api_key):
    response = make_authenticated_get(f"{api_url}/v1/ec2-runner", api_key, timeout=10)
    response_has_success_field = "success" in response.json()
    assert response_has_success_field


def test_ec2_runner_status_has_running_instances_field(api_url, api_key):
    response = make_authenticated_get(f"{api_url}/v1/ec2-runner", api_key, timeout=10)
    response_has_running_instances_field = "running_instances" in response.json()
    assert response_has_running_instances_field


def test_ec2_runner_status_has_instances_field(api_url, api_key):
    response = make_authenticated_get(f"{api_url}/v1/ec2-runner", api_key, timeout=10)
    response_has_instances_field = "instances" in response.json()
    assert response_has_instances_field
...
def test_v1_ec2_runner_post_missing_job_id_returns_400(api_url, api_key):
    ...
    response_is_bad_request = response.status_code == 400
    assert response_is_bad_request
