"""Integration tests for EC2 runner Lambda package contents.
...
def test_ec2_runner_handler_package_includes_runner_labels(config):
    """Verify deployed EC2 runner handler Lambda includes runner_labels.py.
    ...
    """
    function_name = "TenULabsEC2RunnerHandler"
    region = config["aws_region"]
    assert_lambda_package_includes_file(function_name, "runner_labels.py", region)


def test_ec2_runner_handler_processes_labels_without_import_error(config):
    """Verify EC2 runner handler can process events with runner labels.
