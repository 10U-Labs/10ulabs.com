...
def test_simulation_soc_endpoint_accessible_without_auth(api_url):
    response = requests.post(f"{api_url}/v1/simultation-soc", json={"persona": "riscv"}, headers=TEST_HEADERS, timeout=10)
...
def test_simulation_soc_endpoint_returns_200_for_x86_64(api_url):
    response = requests.post(f"{api_url}/v1/simultation-soc", json={"persona": "x86_64"}, headers=TEST_HEADERS, timeout=10)
    is_successful = response.status_code == 200
    assert is_successful


def test_simulation_soc_endpoint_returns_200_for_arm64(api_url):
    response = requests.post(f"{api_url}/v1/simultation-soc", json={"persona": "arm64"}, headers=TEST_HEADERS, timeout=10)
    is_successful = response.status_code == 200
    assert is_successful


def test_simulation_soc_endpoint_returns_400_for_invalid_persona(api_url):
    response = requests.post(f"{api_url}/v1/simultation-soc", json={"persona": "invalid"}, headers=TEST_HEADERS, timeout=10)
...
def test_simulation_soc_endpoint_returns_400_for_missing_persona(api_url):
    response = requests.post(f"{api_url}/v1/simultation-soc", json={}, headers=TEST_HEADERS, timeout=10)
    is_client_error = response.status_code == 400
    assert is_client_error
