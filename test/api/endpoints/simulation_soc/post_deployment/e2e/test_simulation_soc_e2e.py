...
def test_simulation_soc_endpoint_stable_over_sequential_requests(api_url):
    responses = [requests.post(f"{api_url}/v1/simultation-soc", json={"persona": "riscv"}, headers=TEST_HEADERS, timeout=10) for _ in range(5)]
...
def test_simulation_soc_endpoint_consistent_persona_results(api_url):
    responses = [requests.post(f"{api_url}/v1/simultation-soc", json={"persona": "arm64"}, headers=TEST_HEADERS, timeout=10) for _ in range(3)]
...
def test_simulation_soc_endpoint_average_response_time_acceptable(api_url):
    ...
    for _ in range(5):
        start = time.time()
        requests.post(f"{api_url}/v1/simultation-soc", json={"persona": "riscv"}, headers=TEST_HEADERS, timeout=10)
...
def test_simulation_soc_endpoint_no_cold_start_degradation(api_url):
    first_response = requests.post(f"{api_url}/v1/simultation-soc", json={"persona": "riscv"}, headers=TEST_HEADERS, timeout=10)
    time.sleep(1)
    second_response = requests.post(f"{api_url}/v1/simultation-soc", json={"persona": "riscv"}, headers=TEST_HEADERS, timeout=10)
...
def test_simulation_soc_all_personas_return_valid_json(api_url):
    ...
    for persona in personas:
        response = requests.post(f"{api_url}/v1/simultation-soc", json={"persona": persona}, headers=TEST_HEADERS, timeout=10)
        data = response.json()
...
def test_simulation_soc_relative_slowdown_consistent_per_persona(api_url):
    responses = [requests.post(f"{api_url}/v1/simultation-soc", json={"persona": "x86_64"}, headers=TEST_HEADERS, timeout=10) for _ in range(3)]
