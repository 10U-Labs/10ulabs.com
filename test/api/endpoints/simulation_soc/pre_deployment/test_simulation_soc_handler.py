...
def test_handler_returns_200_for_x86_64_persona(
        simulation_soc_handler, simulation_soc_post_event_factory, lambda_context):
    """Verify handler returns 200 for x86_64 persona."""
    event = simulation_soc_post_event_factory(body_data={'persona': 'x86_64'})
    response = simulation_soc_handler.handler(event, lambda_context)
    assert_response_status(response, 200)


def test_handler_returns_200_for_arm64_persona(
        simulation_soc_handler, simulation_soc_post_event_factory, lambda_context):
    """Verify handler returns 200 for ARM64 persona."""
...
def test_tri_mode_ipc_is_less_than_native_ipc_for_x86(
        simulation_soc_handler, simulation_soc_post_event_factory, lambda_context):
    """Verify tri-mode IPC is less than native for x86_64."""
    event = simulation_soc_post_event_factory(body_data={'persona': 'x86_64'})
    response = simulation_soc_handler.handler(event, lambda_context)
    body = parse_response_body(response)
...
def test_relative_slowdown_is_greater_than_one_for_x86(
        simulation_soc_handler, simulation_soc_post_event_factory, lambda_context):
    """Verify relative slowdown is greater than 1 for x86_64."""
    event = simulation_soc_post_event_factory(body_data={'persona': 'x86_64'})
    response = simulation_soc_handler.handler(event, lambda_context)
    body = parse_response_body(response)
...
def test_tri_mode_ipc_is_less_than_native_ipc_for_arm64(
        simulation_soc_handler, simulation_soc_post_event_factory, lambda_context):
    """Verify tri-mode IPC is less than native for ARM64."""
    event = simulation_soc_post_event_factory(body_data={'persona': 'arm64'})
    response = simulation_soc_handler.handler(event, lambda_context)
    body = parse_response_body(response)
    tri_mode_slower = body['tri_mode_core']['ipc'] < body['native_core']['ipc']
...
def test_relative_slowdown_is_greater_than_one_for_arm64(
        simulation_soc_handler, simulation_soc_post_event_factory, lambda_context):
    """Verify relative slowdown is greater than 1 for ARM64."""
    event = simulation_soc_post_event_factory(body_data={'persona': 'arm64'})
    response = simulation_soc_handler.handler(event, lambda_context)
    body = parse_response_body(response)
    slowdown_greater_than_one = body['relative_slowdown'] > 1.0
...
def test_compute_frontend_ipc_trimode_lower_for_x86(simulation_soc_handler):
    """Verify frontend IPC is lower for tri-mode x86_64."""
    ...
    trimode = simulation_soc_handler.compute_frontend_ipc('x86_64', adjusted)
...
def test_compute_trimode_simulation_ipc_less_than_native_for_x86(simulation_soc_handler):
    """Verify tri-mode IPC is lower than native for x86_64."""
...
def test_x86_has_more_uops_than_riscv(simulation_soc_handler):
    """Verify x86_64 generates more micro-ops than RISC-V."""
...
def test_riscv_native_ipc_higher_than_x86(simulation_soc_handler):
    """Verify RISC-V native IPC is higher than x86_64."""
...
def test_compute_flags_overhead_uops_positive_for_x86(simulation_soc_handler):
    """Verify flags overhead is positive for x86_64."""
...
def test_compute_fence_stall_cycles_zero_for_x86_with_hardware_tso(simulation_soc_handler):
    """Verify fence stall cycles are zero for x86 with hardware TSO."""
...
def test_compute_trimode_mispredict_penalty_same_for_all_isas(simulation_soc_handler):
    """Verify mispredict penalty is the same for all ISAs."""
    riscv = simulation_soc_handler.compute_trimode_mispredict_penalty('riscv')
    x86 = simulation_soc_handler.compute_trimode_mispredict_penalty('x86_64')
    arm64 = simulation_soc_handler.compute_trimode_mispredict_penalty('arm64')
    all_equal = riscv == x86 == arm64
    assert all_equal
...
def test_compute_adjusted_uop_stats_x86_higher_than_base(simulation_soc_handler):
    """Verify x86_64 adjusted uops are higher than base due to overhead."""
