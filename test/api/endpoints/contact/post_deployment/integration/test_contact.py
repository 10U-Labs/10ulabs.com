...
def test_contact_endpoint_responds(api_url):
    ...
    response = requests.post(f"{api_url}/v1/contact", headers=INTEGRATION_TEST_HEADERS, json=payload, timeout=10)
...
def test_contact_endpoint_response_time_under_5_seconds(api_url):
    payload = make_contact_request_body()
    start = time.time()
    requests.post(f"{api_url}/v1/contact", headers=INTEGRATION_TEST_HEADERS, json=payload, timeout=10)
