...
class TestLaunchPackerBuilder:
    ...
    def test_successful_workflow_trigger(self, handler_module):
        with patch.object(handler_module, 'trigger_github_workflow', return_value={'success': True}):
            result = handler_module.launch_packer_builder({})

            assert result['success'] is True

    def test_calls_trigger_github_workflow_once(self, handler_module):
        with patch.object(handler_module, 'trigger_github_workflow', return_value={'success': True}) as mock_trigger:
    ...
    def test_error_handling(self, handler_module):
        with patch.object(handler_module, 'trigger_github_workflow', return_value={'success': False, 'error': 'failed'}):
            result = handler_module.launch_packer_builder({})
