...
def test_missing_device_id_returns_400(api_url: str) -> None:
...
def test_missing_configuration_returns_400(api_url: str, test_device_id: str) -> None:
    response = requests.post(
        f"{api_url}/v1/rack-configurations",
...
def test_invalid_configuration_returns_400(api_url: str, test_device_id: str) -> None:
...
def test_get_invalid_format_returns_400(api_url: str) -> None:
...
def test_get_not_found_returns_404(api_url: str) -> None:
    response = requests.get(
...
def test_404_response_is_json(api_url: str) -> None:
    response = requests.get(
        f"{api_url}/v1/rack-configurations/NOTFOUND0",
        timeout=10
    )
...
def test_404_response_does_not_contain_line_numbers(api_url: str) -> None:
    response = requests.get(
        f"{api_url}/v1/rack-configurations/NOTFOUND0",
        timeout=10
    )
...
def test_error_response_is_json(api_url: str) -> None:
    assert error_response_is_json(
        f"{api_url}/v1/rack-configurations")


def test_error_response_has_error_field(api_url: str) -> None:
    assert error_response_names_the_error(
        f"{api_url}/v1/rack-configurations")


def test_error_response_does_not_reveal_var_paths(api_url: str) -> None:
    assert error_response_hides(
        f"{api_url}/v1/rack-configurations", "/var/")


def test_error_response_does_not_reveal_tmp_paths(api_url: str) -> None:
    assert error_response_hides(
