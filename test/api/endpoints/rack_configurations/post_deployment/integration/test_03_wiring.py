...
def test_handler_has_api_gateway_permission(
    lambda_client: Any, handler_function_name: str
) -> None:
    try:
...
def test_handler_has_role_attached(lambda_client: Any, handler_function_name: str) -> None:
...
def test_handler_role_follows_naming_pattern(
    lambda_client: Any, handler_function_name: str, handler_role_name: str
) -> None:
    response = lambda_client.get_function(FunctionName=handler_function_name)
    role_arn = response["Configuration"].get("Role", "")
...
def test_handler_role_has_basic_execution_policy(iam_client: Any, handler_role_name: str) -> None:
...
def test_handler_role_has_dynamodb_policy(iam_client: Any, handler_role_name: str) -> None:
...
def test_backup_role_has_backup_policy(iam_client: Any, backup_role_name: str) -> None:
...
def test_backup_role_has_restore_policy(iam_client: Any, backup_role_name: str) -> None:
    response = iam_client.list_attached_role_policies(RoleName=backup_role_name)
    policy_arns = [p["PolicyArn"] for p in response["AttachedPolicies"]]
    expected = (
