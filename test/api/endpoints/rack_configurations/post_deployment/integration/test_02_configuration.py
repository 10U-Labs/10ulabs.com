...
def test_handler_uses_python_runtime(lambda_client: Any, handler_function_name: str) -> None:
...
def test_handler_uses_arm64_architecture(lambda_client: Any, handler_function_name: str) -> None:
    response = lambda_client.get_function(FunctionName=handler_function_name)
...
def test_handler_has_correct_timeout(lambda_client: Any, handler_function_name: str) -> None:
    response = lambda_client.get_function(FunctionName=handler_function_name)
...
def test_handler_log_group_has_retention_set(handler_log_group: Any) -> None:
    assert handler_log_group["retention"] is not None, (
        f"Log group '{handler_log_group['name']}' should have retention set"
    )


def test_handler_log_group_retention_is_7_days(handler_log_group: Any) -> None:
...
def test_configurations_table_uses_pay_per_request(
    dynamodb_client: Any, configurations_table_name: str
) -> None:
...
def test_backup_plan_exists(backup_plan_id: Optional[str], backup_plan_name: str) -> None:
    assert backup_plan_id is not None, (
        f"Backup plan '{backup_plan_name}' does not exist"
    )


def test_backup_plan_has_daily_schedule(backup_client: Any, backup_plan_id: Optional[str]) -> None:
...
def test_backup_selection_includes_dynamodb_table(
    backup_client: Any, backup_plan_id: Optional[str], configurations_table_arn: str
) -> None:
