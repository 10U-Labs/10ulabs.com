...
def test_create_ec2_user_data_formatting(ec2_runner_handler):
    ...
    with patch.dict('os.environ', {'AWS_REGION': TEST_AWS_REGION}):
        result = fleet_ops.create_ec2_user_data(
...
def test_get_latest_ami_multiple_amis(mock_get_client, ec2_runner_handler):
    ...
    mock_get_client.return_value = mock_ec2
    result = fleet_ops.get_latest_ami()
    is_newest_ami = result == 'ami-new'
    assert is_newest_ami


@patch('fleet_ops.get_ec2_client')
def test_get_latest_ami_no_amis(mock_get_client, ec2_runner_handler):
...
@patch('fleet_ops.get_ec2_client')
def test_get_latest_ami_client_error(mock_get_client, ec2_runner_handler):
...
def test_get_ec2_config_parsing(ec2_runner_handler):
    ...
    with patch.dict('os.environ', {
        ...
        result = fleet_ops.get_ec2_config()
...
def test_create_ec2_user_data_includes_region(ec2_runner_handler):
    ...
    result = fleet_ops.create_ec2_user_data('token', ['label'], 'repo', 'runner')
