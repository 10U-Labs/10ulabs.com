...
def test_runner_creation_requires_auth(api_url):
    ...
    When: A POST request without API key is sent to /v1/runners/ec2
    ...
    response = requests.post(f"{api_url}/v1/runners/ec2", json=payload, timeout=10)
...
def test_ec2_runner_status_requires_auth(api_url):
    ...
    When: A GET request without API key is sent to /v1/runners/ec2
    Then: The request is rejected with 403 Forbidden
    """
    response = requests.get(f"{api_url}/v1/runners/ec2", timeout=10)
    is_forbidden = response.status_code == 403
    assert is_forbidden
...
def test_ec2_runner_status_accepts_valid_api_key(api_url, api_key):
    ...
    When: A GET request with valid API key is sent to /v1/runners/ec2
    Then: The request succeeds with 200 OK
    """
    response = make_authenticated_get(f"{api_url}/v1/runners/ec2", api_key, timeout=10)
...
def test_ec2_runner_status_returns_json(api_url, api_key):
    ...
    When: A GET request is sent to /v1/runners/ec2
...
def test_v1_ec2_runner_post_missing_job_id_returns_400(api_url, api_key):
    ...
    When: A POST request without job_id is sent to /v1/runners/ec2
    ...
    response = make_authenticated_post(
        f"{api_url}/v1/runners/ec2", api_key, json=payload, timeout=10
    )
...
def test_v1_ec2_runner_post_missing_github_repo_returns_400(api_url, api_key):
    ...
    When: A POST request without github_repo is sent to /v1/runners/ec2
    Then: The request fails with 400 Bad Request
    """
