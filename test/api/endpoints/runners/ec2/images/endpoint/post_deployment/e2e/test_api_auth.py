"""Tests for image-for-ec2-runners API authentication and authorization."""
import requests


def test_protected_endpoint_requires_auth(api_url):
    """Verify protected endpoint returns 403 without auth."""
    response = requests.get(f"{api_url}/v1/runners/ec2/images/latest", timeout=10)
...
def test_protected_endpoint_rejects_invalid_api_key(api_url):
    ...
    url = f"{api_url}/v1/runners/ec2/images/latest"
...
def test_ec2_runner_list_requires_auth(api_url):
    ...
    response = requests.get(f"{api_url}/v1/runners/ec2/images", timeout=10)
    assert response.status_code == 403
