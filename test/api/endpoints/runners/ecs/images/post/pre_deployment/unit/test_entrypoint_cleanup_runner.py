...
def test_cleanup_runner_calls_subprocess_run(mock_run, entrypoint):
...
@patch('entrypoint.subprocess.run')
def test_cleanup_runner_calls_config_sh(mock_run, entrypoint):
...
@patch('entrypoint.subprocess.run')
def test_cleanup_runner_passes_remove_flag(mock_run, entrypoint):
...
@patch('entrypoint.subprocess.run')
def test_cleanup_runner_passes_token_flag(mock_run, entrypoint):
...
@patch('entrypoint.subprocess.run')
def test_cleanup_runner_passes_token_value(mock_run, entrypoint):
