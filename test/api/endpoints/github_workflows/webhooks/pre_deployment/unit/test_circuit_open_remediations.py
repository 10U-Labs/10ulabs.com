"""Unit tests for test circuit open remediation."""
import os
...
from .conftest import circuit_open_utils
...
def test_disable_event_source_mappings_lists_mappings():
    ...
    with patch.object(circuit_open_utils, 'get_lambda_client') as mock_get_client:
        mock_lambda = MagicMock()
        mock_lambda.list_event_source_mappings.return_value = {'EventSourceMappings': []}
        mock_get_client.return_value = mock_lambda
        circuit_open_utils.disable_event_source_mappings('test-function')
...
def test_disable_event_source_mappings_counts_disabled():
    ...
    with patch.object(circuit_open_utils, 'get_lambda_client') as mock_get_client:
        ...
        mock_get_client.return_value = mock_lambda
        result = circuit_open_utils.disable_event_source_mappings('test-function')
...
def test_set_lambda_reserved_concurrency_sets_to_zero(circuit_open_remediations):
    """Test set lambda reserved concurrency sets to zero."""
    with patch('boto3.client') as mock_boto_client:
        mock_lambda = MagicMock()
        mock_boto_client.return_value = mock_lambda
        circuit_open_remediations.set_lambda_reserved_concurrency('test-function', 0)
    assert mock_lambda.put_function_concurrency.called


def test_set_lambda_reserved_concurrency_handles_api_error(circuit_open_remediations):
    ...
    with patch('boto3.client') as mock_boto_client:
        ...
        result = circuit_open_remediations.set_lambda_reserved_concurrency('test-function', 0)
    assert result['success'] is False


def test_send_sns_notification_publishes_message(circuit_open_remediations):
    """Test send sns notification publishes message."""
    with patch('boto3.client') as mock_boto_client:
        mock_sns = MagicMock()
        mock_sns.publish.return_value = {'MessageId': 'test-message-id'}
        mock_boto_client.return_value = mock_sns
        circuit_open_remediations.send_sns_notification('arn:aws:sns:test', 'Subject', 'Message')
    assert mock_sns.publish.called


def test_send_sns_notification_includes_subject(circuit_open_remediations):
    """Test send sns notification includes subject."""
    with patch('boto3.client') as mock_boto_client:
        mock_sns = MagicMock()
        mock_sns.publish.return_value = {'MessageId': 'test-message-id'}
        mock_boto_client.return_value = mock_sns
        circuit_open_remediations.send_sns_notification(
...
def test_send_sns_notification_handles_api_error(circuit_open_remediations):
    """Test send sns notification handles api error."""
    with patch('boto3.client') as mock_boto_client:
        mock_boto_client.return_value = create_mock_sns_publish_error()
        result = circuit_open_remediations.send_sns_notification(
            'arn:aws:sns:test', 'Subject', 'Message'
        )
    assert result['success'] is False


def test_record_incident_creates_incident_record(circuit_open_remediations):
    """Test record incident creates incident record."""
    with patch('boto3.client') as mock_boto_client:
        mock_dynamodb = MagicMock()
        mock_boto_client.return_value = mock_dynamodb
        circuit_open_remediations.record_incident('test-table', 'test-alarm', 'test-reason')
    assert mock_dynamodb.put_item.called


def test_record_incident_sets_correct_attributes(circuit_open_remediations):
...
def test_record_incident_handles_dynamodb_error(circuit_open_remediations):
    ...
    with patch('boto3.client') as mock_boto_client:
        ...
        mock_boto_client.return_value = mock_dynamodb
        result = circuit_open_remediations.record_incident(
            'test-table', 'test-alarm', 'test-reason'
        )
    assert result['success'] is False


def test_update_circuit_open_state_sets_open_state(circuit_open_remediations):
    """Test update circuit open state sets open state."""
    with patch('boto3.client') as mock_boto_client:
        mock_dynamodb = MagicMock()
        mock_boto_client.return_value = mock_dynamodb
        circuit_open_remediations.update_circuit_open_state('test-table', 'open')
        call_args = mock_dynamodb.put_item.call_args
    assert call_args[1]['Item']['state']['S'] == 'open'


def test_update_circuit_open_state_resets_recovery_attempts(circuit_open_remediations):
    """Test update circuit open state resets recovery attempts."""
    with patch('boto3.client') as mock_boto_client:
        mock_dynamodb = MagicMock()
        mock_boto_client.return_value = mock_dynamodb
        circuit_open_remediations.update_circuit_open_state('test-table', 'open')
...
def test_update_circuit_open_state_sets_ttl(circuit_open_remediations):
    """Test update circuit open state sets ttl."""
    with patch('boto3.client') as mock_boto_client:
        mock_dynamodb = MagicMock()
        mock_boto_client.return_value = mock_dynamodb
        circuit_open_remediations.update_circuit_open_state('test-table', 'open')
...
def test_update_circuit_open_state_handles_dynamodb_error(circuit_open_remediations):
    """Test update circuit open state handles dynamodb error."""
    with patch('boto3.client') as mock_boto_client:
        mock_dynamodb = MagicMock()
        mock_dynamodb.put_item.side_effect = ClientError(
            {'Error': {'Code': 'ServiceUnavailable'}},
        ...
        mock_boto_client.return_value = mock_dynamodb
        result = circuit_open_remediations.update_circuit_open_state('test-table', 'open')
    assert result['success'] is False


def test_handle_cloudwatch_alarm_event_parses_alarm_details(circuit_open_remediations):
    """Test handle cloudwatch alarm event parses alarm details."""
    with _patched_remediation_env(_full_env()):
        result = circuit_open_remediations.handle_cloudwatch_alarm_event(_create_alarm_event())
    assert result['alarm_name'] == 'test-alarm'


def test_handle_cloudwatch_alarm_event_skips_insufficient_data_state(circuit_open_remediations):
    ...
    with _patched_remediation_env(_full_env()):
        result = circuit_open_remediations.handle_cloudwatch_alarm_event(event)
    assert 'message' in result


def test_handle_cloudwatch_alarm_event_disables_event_sources(circuit_open_remediations):
...
def test_handle_cloudwatch_alarm_event_sets_zero_concurrency(circuit_open_remediations):
...
def test_handle_cloudwatch_alarm_event_sends_notification(circuit_open_remediations):
...
def test_handle_cloudwatch_alarm_event_records_incident(circuit_open_remediations):
...
def test_handle_cloudwatch_alarm_event_updates_state_table(circuit_open_remediations):
    """Test handle cloudwatch alarm event updates state table."""
    with _patched_remediation_env(_base_env()) as mock_boto:
        mock_boto.return_value = _create_mock_lambda_empty_mappings()
        result = circuit_open_remediations.handle_cloudwatch_alarm_event(_create_alarm_event())
    actions_str = str(result.get('actions_taken', []))
    assert 'circuit open state to OPEN' in actions_str


def test_handle_cloudwatch_alarm_event_tracks_all_actions(circuit_open_remediations):
...
def test_handle_cloudwatch_alarm_event_handles_missing_config(circuit_open_remediations):
...
def test_lambda_handler_routes_cloudwatch_alarm_events(circuit_open_remediations, lambda_context):
    """Test lambda handler routes cloudwatch alarm events."""
    with _patched_remediation_env(_base_env()):
        response = circuit_open_remediations.lambda_handler(_create_alarm_event(), lambda_context)
...
def test_lambda_handler_handles_unsupported_event_types(
    ...
    with patch('boto3.client'):
        response = circuit_open_remediations.lambda_handler(event, lambda_context)
    assert response['statusCode'] == 200
...
def test_remove_lambda_reserved_concurrency_deletes_concurrency(circuit_open_remediations):
    """Test remove lambda reserved concurrency deletes concurrency."""
    with patch('boto3.client') as mock_boto_client:
        mock_lambda = MagicMock()
        mock_boto_client.return_value = mock_lambda
        circuit_open_remediations.remove_lambda_reserved_concurrency('test-function')
    assert mock_lambda.delete_function_concurrency.called


def test_remove_lambda_reserved_concurrency_returns_success(circuit_open_remediations):
    """Test remove lambda reserved concurrency returns success."""
    with patch('boto3.client') as mock_boto_client:
        mock_lambda = MagicMock()
        mock_boto_client.return_value = mock_lambda
        result = circuit_open_remediations.remove_lambda_reserved_concurrency('test-function')
    assert result['success'] is True


def test_remove_lambda_reserved_concurrency_handles_api_error(circuit_open_remediations):
...
def _run_handle_alarm_ok(circuit_open_remediations, sns_topic=None, mock_override=None):
    ...
    with patch('boto3.client') as mock_boto:
        if mock_override:
            ...
            mock_boto.return_value = _create_mock_lambda_empty_mappings()
        circuit_open_remediations.handle_alarm_ok_state(
            result, 'test-function', 'test-state', sns_topic, 'test-alarm'
        )
    return result


def test_handle_alarm_ok_state_enables_event_sources(circuit_open_remediations):
    """Test handle alarm ok state enables event sources."""
    mock_lambda = create_mock_lambda_with_disabled_mappings()
    result = _run_handle_alarm_ok(circuit_open_remediations, mock_override=mock_lambda)
    actions_str = str(result.get('actions_taken', []))
    assert 'Enabled SQS event sources' in actions_str


def test_handle_alarm_ok_state_removes_concurrency_limit(circuit_open_remediations):
    """Test handle alarm ok state removes concurrency limit."""
    result = _run_handle_alarm_ok(circuit_open_remediations)
    actions_str = str(result.get('actions_taken', []))
    assert 'Removed Lambda reserved concurrency limit' in actions_str


def test_handle_alarm_ok_state_resets_circuit_open_to_closed(circuit_open_remediations):
    """Test handle alarm ok state resets circuit open to closed."""
    result = _run_handle_alarm_ok(circuit_open_remediations)
    actions_str = str(result.get('actions_taken', []))
    assert 'Reset circuit open state to CLOSED' in actions_str


def test_handle_alarm_ok_state_sends_recovery_notification(circuit_open_remediations):
    """Test handle alarm ok state sends recovery notification."""
    mock_client = _create_mock_lambda_empty_mappings()
    mock_client.publish.return_value = {'MessageId': 'test-id'}
    result = _run_handle_alarm_ok(circuit_open_remediations, 'arn:aws:sns:test', mock_client)
    actions_str = str(result.get('actions_taken', []))
    assert 'recovery notification' in actions_str


def test_handle_cloudwatch_alarm_event_handles_ok_state(circuit_open_remediations):
    """Test handle cloudwatch alarm event handles ok state."""
    event = _create_alarm_event('OK', 'Back to normal')
    with _patched_remediation_env(_base_env()) as mock_boto:
        mock_boto.return_value = _create_mock_lambda_empty_mappings()
        result = circuit_open_remediations.handle_cloudwatch_alarm_event(event)
    assert result['message'] == 'Circuit open reset to closed state'


def test_handle_cloudwatch_alarm_event_ok_state_updates_dynamodb(circuit_open_remediations):
...
def test_update_circuit_open_state_sets_closed_state(circuit_open_remediations):
    """Test update circuit open state sets closed state."""
    with patch('boto3.client') as mock_boto_client:
        mock_dynamodb = MagicMock()
        mock_boto_client.return_value = mock_dynamodb
        circuit_open_remediations.update_circuit_open_state('test-table', 'closed')
        call_args = mock_dynamodb.put_item.call_args
