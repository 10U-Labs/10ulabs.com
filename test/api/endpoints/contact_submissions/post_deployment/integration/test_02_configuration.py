...
def test_contact_handler_uses_python_runtime(
    lambda_client: Any, shared_config: Dict[str, Any]
) -> None:
...
def test_contact_handler_has_contact_email_env_var(contact_handler_env_vars: Any) -> None:
    assert "CONTACT_EMAIL" in contact_handler_env_vars, (
        "Lambda missing CONTACT_EMAIL environment variable"
    )


def test_contact_handler_has_recaptcha_param_env_var(contact_handler_env_vars: Any) -> None:
...
def test_contact_handler_log_group_has_retention_set(contact_handler_log_group: Any) -> None:
...
def test_contact_handler_log_group_retention_is_7_days(contact_handler_log_group: Any) -> None:
