...
def test_contact_form_submission_returns_success(api_url: str) -> None:
...
def test_contact_form_submission_returns_json(api_url: str) -> None:
    payload = create_contact_payload()
    response = requests.post(
        f"{api_url}/v1/contact-submissions",
        headers=TEST_HEADERS, json=payload, timeout=10
...
def test_contact_form_submission_confirms_test_mode(api_url: str) -> None:
    payload = create_contact_payload()
    response = requests.post(
        f"{api_url}/v1/contact-submissions",
        headers=TEST_HEADERS, json=payload, timeout=10
...
def test_options_request_returns_200(api_url: str) -> None:
...
def test_cors_header_present_in_response(api_url: str) -> None:
    payload = create_contact_payload()
    response = requests.post(
        f"{api_url}/v1/contact-submissions",
        headers=TEST_HEADERS, json=payload, timeout=10
...
def test_response_time_under_5_seconds(api_url: str) -> None:
    payload = create_contact_payload()
    start = time.time()
    requests.post(
        f"{api_url}/v1/contact-submissions",
...
def test_stable_over_multiple_requests(api_url: str) -> None:
    payload = create_contact_payload()
    statuses = []
    for _ in range(3):
        response = requests.post(
...
def test_missing_name_returns_400(api_url: str) -> None:
    payload = create_contact_payload()
    payload["name"] = ""
    response = requests.post(
        f"{api_url}/v1/contact-submissions",
...
def test_invalid_email_returns_400(api_url: str) -> None:
    payload = create_contact_payload()
