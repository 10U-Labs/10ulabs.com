...
def test_missing_device_id(handler: ModuleType) -> None:
    body = {'events': [{'event_type': 'test', 'timestamp': '2024-01-15T10:30:00Z'}]}
    result = handler.validate_analytics_request(body)
    assert 'device_id' in result


def test_missing_events(handler: ModuleType) -> None:
    body = {'device_id': 'device123'}
    result = handler.validate_analytics_request(body)
    assert 'events' in result


def test_empty_events_array(handler: ModuleType) -> None:
    body = {'device_id': 'device123', 'events': []}
    result = handler.validate_analytics_request(body)
    assert 'empty' in result


def test_too_many_events(handler: ModuleType) -> None:
...
def test_options_returns_200(handler: ModuleType) -> None:
    event = {'httpMethod': 'OPTIONS', 'path': '/v1/sessions/abc123/events'}
    response = handler.lambda_handler(event, None)
    assert response['statusCode'] == 200


def test_options_returns_cors_headers(handler: ModuleType) -> None:
    event = {'httpMethod': 'OPTIONS', 'path': '/v1/sessions/abc123/events'}
    response = handler.lambda_handler(event, None)
    assert 'Access-Control-Allow-Origin' in response['headers']


def test_unknown_path_returns_404(handler: ModuleType) -> None:
    event = {'httpMethod': 'GET', 'path': '/v1/unknown'}
    response = handler.lambda_handler(event, None)
    assert response['statusCode'] == 404


def test_returns_empty_dict_when_body_is_empty_string(handler: ModuleType) -> None:
    event = {'body': ''}
    result = handler.parse_body(event)
    assert result == {}


def test_returns_parsed_json_from_string_body(handler: ModuleType) -> None:
    event = {'body': '{"key": "value"}'}
    result = handler.parse_body(event)
    assert result == {'key': 'value'}


def test_handles_base64_encoded_body(handler: ModuleType) -> None:
...
def test_event_type_not_string_returns_error(handler: ModuleType) -> None:
    event = {'event_type': 123, 'timestamp': '2024-01-15T10:30:00Z'}
    result = handler.validate_analytics_event(event)
    assert 'string' in result


def test_timestamp_not_string_returns_error(handler: ModuleType) -> None:
    event = {'event_type': 'page_view', 'timestamp': 1234567890}
    result = handler.validate_analytics_event(event)
    assert 'string' in result


def test_device_id_not_string_returns_error(handler: ModuleType) -> None:
    body = {
        'device_id': 12345,
        'events': [{'event_type': 'test', 'timestamp': '2024-01-15T10:30:00Z'}]
    }
...
def test_events_not_array_returns_error(handler: ModuleType) -> None:
    body = {
        'device_id': 'device123',
        'events': {'event_type': 'test', 'timestamp': '2024-01-15T10:30:00Z'}
    }
...
def test_includes_content_type_header(handler: ModuleType) -> None:
    response = handler.json_response(200, {'success': True})
    assert response['headers']['Content-Type'] == 'application/json'


def test_includes_cors_allow_origin_header(handler: ModuleType) -> None:
    response = handler.json_response(200, {'success': True})
    assert response['headers']['Access-Control-Allow-Origin'] == '*'


def test_includes_cors_allow_methods_get(handler: ModuleType) -> None:
    response = handler.json_response(200, {'success': True})
    assert 'GET' in response['headers']['Access-Control-Allow-Methods']


def test_includes_cors_allow_methods_post(handler: ModuleType) -> None:
    response = handler.json_response(200, {'success': True})
    assert 'POST' in response['headers']['Access-Control-Allow-Methods']


def test_body_is_json_string(handler: ModuleType) -> None:
    response = handler.json_response(200, {'key': 'value'})
    parsed = json.loads(response['body'])
    assert parsed == {'key': 'value'}


def test_includes_details_when_provided(handler: ModuleType) -> None:
    response = handler.error_response(400, 'Bad request', 'Missing field')
    body = json.loads(response['body'])
    assert body['details'] == 'Missing field'


def test_excludes_details_when_empty(handler: ModuleType) -> None:
    response = handler.error_response(400, 'Bad request', '')
    body = json.loads(response['body'])
    assert 'details' not in body


def test_includes_success_false(handler: ModuleType) -> None:
    response = handler.error_response(400, 'Bad request')
    body = json.loads(response['body'])
