"""Pre-deployment tests for api/common/networking VPC Terraform configuration."""
import re


def test_vpc_tf_defines_vpc(api_common_networking_dir):
...
def test_vpc_has_correct_cidr(api_common_networking_dir):
...
def test_vpc_has_dns_support(api_common_networking_dir):
...
def test_vpc_has_dns_hostnames(api_common_networking_dir):
...
def test_vpc_has_public_subnets(api_common_networking_dir):
...
def test_vpc_has_internet_gateway(api_common_networking_dir):
...
def test_security_group_defined(api_common_networking_dir):
...
def test_security_group_has_egress_block(api_common_networking_dir):
...
def test_security_group_allows_all_outbound(api_common_networking_dir):
