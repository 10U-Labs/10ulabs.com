"""Pre-deployment unit tests for api/common/docker_repository outputs.tf configuration."""
import re


def test_ecr_repository_arn_output_exists(api_common_docker_repository_dir):
    """Test that ecr_repository_arn output is defined."""
    content = (api_common_docker_repository_dir / "outputs.tf").read_text()
    pattern = r'output\s+"ecr_repository_arn"'
    assert re.search(pattern, content) is not None


def test_ecr_repository_arn_references_runners(api_common_docker_repository_dir):
    """Test that ecr_repository_arn references aws_ecr_repository.runners.arn."""
    content = (api_common_docker_repository_dir / "outputs.tf").read_text()
    assert "aws_ecr_repository.runners.arn" in content


def test_ecr_repository_name_output_exists(api_common_docker_repository_dir):
...
def test_ecr_repository_name_references_runners(api_common_docker_repository_dir):
...
def test_ecr_repository_url_output_exists(api_common_docker_repository_dir):
...
def test_ecr_repository_url_references_runners(api_common_docker_repository_dir):
