"""Pre-deployment unit tests for api/common/docker_repository ECR Terraform configuration."""
import re


def test_ecr_tf_defines_runners_repository(api_common_docker_repository_dir):
    """Test that ecr.tf defines the runners ECR repository."""
    ecr_tf = api_common_docker_repository_dir / "ecr.tf"
...
def test_ecr_repository_name_matches_shared_module(
    api_common_docker_repository_dir, shared_module_dir
):
    ...
    locals_tf = api_common_docker_repository_dir / "locals.tf"
    content = locals_tf.read_text()
    assert f'ecr_repository_name = "{expected_name}"' in content


def test_ecr_repository_has_scan_on_push_disabled(api_common_docker_repository_dir):
...
def test_ecr_repository_has_aes256_encryption(api_common_docker_repository_dir):
...
def test_ecr_repository_has_mutable_image_tags(api_common_docker_repository_dir):
...
def test_ecr_repository_uses_common_tags(api_common_docker_repository_dir):
...
def test_common_tags_has_managed_by_terraform(api_common_docker_repository_dir):
...
def test_ecr_lifecycle_policy_defined(api_common_docker_repository_dir):
...
def test_ecr_lifecycle_has_latest_rule(api_common_docker_repository_dir):
...
def test_ecr_lifecycle_has_stable_rule(api_common_docker_repository_dir):
