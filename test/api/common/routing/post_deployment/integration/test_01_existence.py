...
def test_lambda_catchall_handler_exists(lambda_client: Any, shared_config: Dict[str, Any]) -> None:
    function_name = shared_config['lambda_handler_names']['catchall']
    response = lambda_client.get_function(FunctionName=function_name)
    assert response["Configuration"]["FunctionName"] == function_name


def test_catchall_handler_role_exists(iam_client: Any, shared_config: Dict[str, Any]) -> None:
...
def test_api_gateway_api_key_exists(apigateway_client: Any, api_gateway_id: Optional[str]) -> None:
...
def test_s3_docs_bucket_exists(s3_client: Any, config: Dict[str, Any]) -> None:
    bucket_name = config["api_fqdn"]
    response = s3_client.head_bucket(Bucket=bucket_name)
    assert response["ResponseMetadata"]["HTTPStatusCode"] == 200


def test_index_html_exists_in_s3(s3_client: Any, config: Dict[str, Any]) -> None:
    bucket_name = config["api_fqdn"]
    response = s3_client.head_object(Bucket=bucket_name, Key="index.html")
    assert response["ResponseMetadata"]["HTTPStatusCode"] == 200


def test_openapi_json_exists_in_s3(s3_client: Any, config: Dict[str, Any]) -> None:
    bucket_name = config["api_fqdn"]
    response = s3_client.head_object(Bucket=bucket_name, Key="openapi.json")
    assert response["ResponseMetadata"]["HTTPStatusCode"] == 200


def test_404_html_exists_in_s3(s3_client: Any, config: Dict[str, Any]) -> None:
    bucket_name = config["api_fqdn"]
    response = s3_client.head_object(Bucket=bucket_name, Key="404.html")
    assert response["ResponseMetadata"]["HTTPStatusCode"] == 200


def test_s3_bucket_policy_exists(s3_client: Any, config: Dict[str, Any]) -> None:
    bucket_name = config["api_fqdn"]
...
def test_cloudfront_origin_access_control_exists(cloudfront_client: Any) -> None:
...
def test_route53_api_record_exists(api_route53_records: Any, config: Dict[str, Any]) -> None:
...
def test_firehose_delivery_stream_exists(firehose_client: Any, config: Dict[str, Any]) -> None:
...
def test_firehose_role_exists(iam_client: Any, config: Dict[str, Any]) -> None:
    response = iam_client.get_role(RoleName=config['firehose_role_name'])
    assert response['Role']['RoleName'] == config['firehose_role_name']


def test_cloudwatch_logs_firehose_role_exists(iam_client: Any, config: Dict[str, Any]) -> None:
    role_name = config['cloudwatch_logs_firehose_role_name']
    response = iam_client.get_role(RoleName=role_name)
    assert response['Role']['RoleName'] == role_name


def test_api_key_ssm_parameter_exists(ssm_client: Any, config: Dict[str, Any]) -> None:
...
def test_api_gateway_rest_api_exists(apigateway_client: Any, config: Dict[str, Any]) -> None:
    apis = apigateway_client.get_rest_apis()
    api_names = [api['name'] for api in apis['items']]
    assert config['api_gateway_name'] in api_names


def test_api_gateway_stage_exists(apigateway_client: Any, api_gateway_id: Optional[str]) -> None:
    if api_gateway_id is None:
        pytest.skip("API Gateway not found")
...
def test_api_gateway_cloudwatch_role_exists(iam_client: Any, config: Dict[str, Any]) -> None:
    role_name = config['api_gateway_cloudwatch_role_name']
    response = iam_client.get_role(RoleName=role_name)
    assert response['Role']['RoleName'] == role_name


def test_cloudfront_url_rewrite_function_exists(cloudfront_client: Any) -> None:
