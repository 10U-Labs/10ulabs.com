...
def test_s3_bucket_versioning_disabled(s3_client: Any, config: Dict[str, Any]) -> None:
    bucket_name = config["api_fqdn"]
    response = s3_client.get_bucket_versioning(Bucket=bucket_name)
    assert response.get("Status") != "Enabled"


def test_s3_bucket_encryption_config_exists(s3_client: Any, config: Dict[str, Any]) -> None:
    bucket_name = config["api_fqdn"]
    response = s3_client.get_bucket_encryption(Bucket=bucket_name)
    assert "ServerSideEncryptionConfiguration" in response


def test_s3_bucket_encryption_has_rules(s3_client: Any, config: Dict[str, Any]) -> None:
    bucket_name = config["api_fqdn"]
    response = s3_client.get_bucket_encryption(Bucket=bucket_name)
    assert "Rules" in response["ServerSideEncryptionConfiguration"]


def test_cloudfront_distribution_has_default_cache_behavior(
    first_cloudfront_dist_config: Any
) -> None:
    if first_cloudfront_dist_config is not None:
        assert 'DefaultCacheBehavior' in first_cloudfront_dist_config


def test_cloudfront_distribution_allows_get_head_methods(first_cloudfront_dist_config: Any) -> None:
    if first_cloudfront_dist_config is not None:
...
def test_cloudfront_distribution_has_viewer_protocol_policy(
    first_cloudfront_dist_config: Any
) -> None:
    if first_cloudfront_dist_config is not None:
        cache_behavior = first_cloudfront_dist_config['DefaultCacheBehavior']
...
def test_cloudfront_distribution_compression_enabled(first_cloudfront_dist_config: Any) -> None:
    if first_cloudfront_dist_config is not None:
        assert 'Compress' in first_cloudfront_dist_config['DefaultCacheBehavior']


def test_cloudfront_distribution_logging_enabled(first_cloudfront_dist_config: Any) -> None:
    if first_cloudfront_dist_config is not None:
        logging_config = first_cloudfront_dist_config.get('Logging', {})
        assert logging_config.get('Enabled', False)


def test_cloudfront_logging_excludes_cookies(first_cloudfront_dist_config: Any) -> None:
    if first_cloudfront_dist_config is not None:
        logging_config = first_cloudfront_dist_config.get('Logging', {})
        assert logging_config.get('IncludeCookies', True) is False


def test_cloudfront_viewer_certificate_min_protocol_tls_1_2(
    first_cloudfront_dist_config: Any
) -> None:
    if first_cloudfront_dist_config is not None:
...
def test_cloudfront_geo_restriction_is_none(first_cloudfront_dist_config: Any) -> None:
    if first_cloudfront_dist_config is not None:
...
def test_cloudfront_distribution_ipv6_disabled(first_cloudfront_dist_config: Any) -> None:
    if first_cloudfront_dist_config is not None:
        assert first_cloudfront_dist_config.get('IsIPV6Enabled') is False


def test_cloudfront_distribution_http_version_is_http2(first_cloudfront_dist_config: Any) -> None:
    if first_cloudfront_dist_config is not None:
        http_version = first_cloudfront_dist_config.get('HttpVersion', '')
        assert 'http2' in http_version


def test_cloudfront_aliases_include_api_fqdn(
...
def test_cloudfront_api_origin_protocol_is_https_only(
    cloudfront_client: Any,
    api_distribution_id: Optional[str]
) -> None:
...
def test_cloudfront_health_cache_behavior_uses_api_origin(
    cloudfront_client: Any,
    api_distribution_id: Optional[str]
) -> None:
...
def test_cloudfront_diagnostics_cache_behavior_targets_api_10ulabs_com(
    cloudfront_client: Any,
    api_distribution_id: Optional[str]
) -> None:
    behavior = _cache_behavior_matching(cloudfront_client, api_distribution_id, '/diagnostics')
    assert behavior['TargetOriginId'] == 'api-10ulabs-com'
...
def test_cloudfront_docs_cache_behavior_uses_s3_origin(
    cloudfront_client: Any,
    api_distribution_id: Optional[str]
) -> None:
    dist_config = _api_distribution_config(cloudfront_client, api_distribution_id)
...
def test_cloudfront_distribution_price_class(
    cloudfront_client: Any,
    api_distribution_id: Optional[str]
) -> None:
    dist_config = _api_distribution_config(cloudfront_client, api_distribution_id)
...
def test_acm_certificate_is_issued(acm_client: Any) -> None:
    certificates = acm_client.list_certificates(CertificateStatuses=['ISSUED'])
    assert len(certificates['CertificateSummaryList']) > 0


def test_acm_certificate_validation_method_is_dns(acm_client: Any) -> None:
    certificates = acm_client.list_certificates(CertificateStatuses=['ISSUED'])
...
def test_acm_certificate_domain_name_matches(acm_client: Any, config: Dict[str, Any]) -> None:
    certificates = acm_client.list_certificates(CertificateStatuses=['ISSUED'])
    if not certificates['CertificateSummaryList']:
        pytest.skip("No issued certificates found")
...
def _find_docs_cache_policy(cloudfront_client: Any) -> Any:
...
def test_cloudfront_docs_cache_policy_default_ttl(cloudfront_client: Any) -> None:
...
def test_cloudfront_docs_cache_policy_max_ttl(cloudfront_client: Any) -> None:
    docs_policy = _find_docs_cache_policy(cloudfront_client)
    if docs_policy:
        config = docs_policy['CachePolicy']['CachePolicyConfig']
...
def test_cloudfront_docs_cache_policy_min_ttl(cloudfront_client: Any) -> None:
    docs_policy = _find_docs_cache_policy(cloudfront_client)
    if docs_policy:
        config = docs_policy['CachePolicy']['CachePolicyConfig']
...
def test_cloudfront_docs_cache_policy_gzip_enabled(cloudfront_client: Any) -> None:
    docs_policy = _find_docs_cache_policy(cloudfront_client)
    if docs_policy:
        config = docs_policy['CachePolicy']['CachePolicyConfig']
...
def test_cloudfront_docs_cache_policy_brotli_enabled(cloudfront_client: Any) -> None:
    docs_policy = _find_docs_cache_policy(cloudfront_client)
    if docs_policy:
        config = docs_policy['CachePolicy']['CachePolicyConfig']
        params = config['ParametersInCacheKeyAndForwardedToOrigin']
...
def test_cloudfront_docs_cache_policy_cookies_none(cloudfront_client: Any) -> None:
    response = cloudfront_client.list_cache_policies(Type='custom')
    policies = response['CachePolicyList'].get('Items', [])
    docs_policy = next(
...
def test_firehose_delivery_stream_is_active(firehose_client: Any, config: Dict[str, Any]) -> None:
    stream_name = config['firehose_delivery_stream_name']
    response = firehose_client.describe_delivery_stream(DeliveryStreamName=stream_name)
    assert response['DeliveryStreamDescription']['DeliveryStreamStatus'] == 'ACTIVE'


def test_firehose_delivery_stream_type_is_direct_put(
    firehose_client: Any,
    config: Dict[str, Any]
) -> None:
    stream_name = config['firehose_delivery_stream_name']
...
def test_firehose_destination_is_extended_s3(firehose_client: Any, config: Dict[str, Any]) -> None:
    stream_name = config['firehose_delivery_stream_name']
    response = firehose_client.describe_delivery_stream(DeliveryStreamName=stream_name)
    destinations = response['DeliveryStreamDescription']['Destinations']
...
def test_firehose_buffering_interval_is_300_seconds(
    firehose_client: Any,
    config: Dict[str, Any]
) -> None:
    s3_config = _firehose_s3_destination(firehose_client, config)
...
def test_api_key_ssm_parameter_is_secure_string(ssm_client: Any, config: Dict[str, Any]) -> None:
...
def test_api_gateway_rest_api_endpoint_type_is_regional(
...
def test_api_gateway_stage_access_log_format_configured(
    apigateway_client: Any,
    api_gateway_id: Optional[str]
) -> None:
...
def test_api_gateway_method_settings_logging_level_is_info(
    apigateway_client: Any,
    api_gateway_id: Optional[str]
) -> None:
...
def test_api_gateway_method_settings_metrics_enabled(
    apigateway_client: Any,
    api_gateway_id: Optional[str]
) -> None:
    method_settings = _stage_method_settings(apigateway_client, api_gateway_id)
...
def test_api_gateway_method_settings_data_trace_enabled(
    apigateway_client: Any,
    api_gateway_id: Optional[str]
) -> None:
    method_settings = _stage_method_settings(apigateway_client, api_gateway_id)
...
def test_api_gateway_method_settings_apply_to_all_methods(
    apigateway_client: Any,
    api_gateway_id: Optional[str]
) -> None:
    method_settings = _stage_method_settings(apigateway_client, api_gateway_id)
...
def test_api_gateway_api_key_is_enabled(apigateway_client: Any) -> None:
    response = apigateway_client.get_api_keys()
    if response['items']:
        assert response['items'][0]['enabled'] is True


def test_api_gateway_usage_plan_throttle_rate_limit(apigateway_client: Any) -> None:
...
def test_api_gateway_stage_has_logging_enabled(
    apigateway_client: Any,
    api_gateway_id: Optional[str]
) -> None:
    response = _prod_stage(apigateway_client, api_gateway_id)
...
def test_api_gateway_stage_has_xray_tracing_disabled(
    apigateway_client: Any,
    api_gateway_id: Optional[str]
) -> None:
    response = _prod_stage(apigateway_client, api_gateway_id)
...
def test_cloudfront_url_rewrite_function_runtime(cloudfront_client: Any) -> None:
...
def test_firehose_cloudwatch_logs_bucket_is_central_logs(
    firehose_client: Any,
    config: Dict[str, Any]
) -> None:
    s3_config = _firehose_s3_destination(firehose_client, config)
...
def _check_firehose_cw_logging_disabled(response: Any) -> bool:
    destinations = response['DeliveryStreamDescription']['Destinations']
...
def test_firehose_cloudwatch_logs_cloudwatch_logging_disabled(
    firehose_client: Any,
    config: Dict[str, Any]
) -> None:
    stream_name = config['firehose_delivery_stream_name']
...
def test_catchall_subscription_filter_pattern_is_empty(
    logs_client: Any,
    config: Dict[str, Any]
) -> None:
...
def test_api_gateway_subscription_filter_pattern_is_empty(
    logs_client: Any,
    config: Dict[str, Any]
) -> None:
...
def test_404_html_content_type_is_text_html(s3_client: Any, config: Dict[str, Any]) -> None:
    bucket_name = config["api_fqdn"]
    response = s3_client.head_object(Bucket=bucket_name, Key="404.html")
    assert response['ContentType'] == 'text/html'


def test_openapi_json_content_type_is_application_json(
    s3_client: Any,
    config: Dict[str, Any]
) -> None:
    bucket_name = config["api_fqdn"]
...
def test_s3_docs_bucket_logging_enabled(s3_client: Any, config: Dict[str, Any]) -> None:
    bucket_name = config["api_fqdn"]
    try:
...
def test_catchall_handler_log_group_retention_is_7_days(
    logs_client: Any,
    config: Dict[str, Any]
) -> None:
    log_group = config['catchall_handler_log_group_name']
...
def test_api_gateway_log_group_retention_is_30_days(
    logs_client: Any,
    config: Dict[str, Any]
) -> None:
    log_group = config['api_gateway_log_group_name']
...
def test_catchall_handler_log_group_kms_encryption(
    logs_client: Any,
    config: Dict[str, Any]
) -> None:
    log_group = config['catchall_handler_log_group_name']
...
def test_api_gateway_log_group_kms_encryption(logs_client: Any, config: Dict[str, Any]) -> None:
    log_group = config['api_gateway_log_group_name']
    response = logs_client.describe_log_groups(logGroupNamePrefix=log_group)
    if response['logGroups']:
        kms_key = response['logGroups'][0].get('kmsKeyId')
