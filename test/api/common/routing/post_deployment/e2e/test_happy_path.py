...
def test_root_endpoint_responds(api_url: str) -> None:
    response = requests.get(api_url, headers=TEST_HEADERS, timeout=10)
    assert response.status_code in [200, 301, 302]


def test_openapi_spec_accessible(api_url: str) -> None:
    response = requests.get(f"{api_url}/openapi.json", headers=TEST_HEADERS, timeout=10)
    assert response.status_code == 200


def test_openapi_spec_is_valid_json(api_url: str) -> None:
    response = requests.get(f"{api_url}/openapi.json", headers=TEST_HEADERS, timeout=10)
    content_type = response.headers.get("Content-Type", "")
    assert "application/json" in content_type


def test_health_endpoint_responds(api_url: str) -> None:
...
def test_api_handles_concurrent_requests(api_url: str) -> None:
    ...
    def make_health_request() -> Optional[int]:
        try:
...
def test_lambda_cold_start_responds_successfully(api_url: str) -> None:
    skip_if_endpoint_not_deployed(api_url, "/health")
    response = requests.get(f"{api_url}/health", headers=TEST_HEADERS, timeout=15)
    assert response.status_code == 200


def test_lambda_cold_start_performance(api_url: str) -> None:
    skip_if_endpoint_not_deployed(api_url, "/health")
    start = time.time()
