...
def test_malformed_json_request_returns_400(api_url):
    ...
    response = requests.post(
        f"{api_url}/diagnostics/echo", data="not valid json", headers=headers, timeout=10
    )
...
def test_oversized_payload_returns_413(api_url):
    ...
    response = requests.post(
        f"{api_url}/diagnostics/echo", json=large_payload, headers=TEST_HEADERS, timeout=30
    )
...
def test_api_handles_malformed_json_gracefully(api_url):
    ...
    headers = {"Content-Type": "application/json", "x-test-mode": "true"}
    response = requests.post(
        f"{api_url}/diagnostics/echo", data="{invalid json", headers=headers, timeout=10
    )
    assert response.status_code in [400, 500]
