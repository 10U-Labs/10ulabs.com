...
class TestDeployedIAMRoleNamingConventions:
    ...
    def test_health_handler_role_exists(self, iam_client, config):
...
class TestDeployedLambdaFunctionNamingConventions:
    ...
    def test_health_handler_function_exists(self, lambda_client, config):
        """Verify HealthHandler Lambda function exists."""
        function_name = config.get('health_handler_function_name', 'TenULabsHealthHandler')
        try:
    ...
    def test_health_handler_function_name_is_pascalcase(self, lambda_client, config):
