...
def test_central_logs_bucket_encryption_is_aes256(s3_client: Any, config: Dict[str, Any]) -> None:
    bucket_name = config['name_for_central_logs_bucket']
    encryption = s3_client.get_bucket_encryption(Bucket=bucket_name)
...
def test_central_logs_bucket_blocks_public_acls(s3_client: Any, config: Dict[str, Any]) -> None:
    bucket_name = config['name_for_central_logs_bucket']
...
def test_central_logs_bucket_blocks_public_policy(s3_client: Any, config: Dict[str, Any]) -> None:
    bucket_name = config['name_for_central_logs_bucket']
    public_access = s3_client.get_public_access_block(Bucket=bucket_name)
    block_config = public_access['PublicAccessBlockConfiguration']
...
def test_central_logs_bucket_ignores_public_acls(s3_client: Any, config: Dict[str, Any]) -> None:
    bucket_name = config['name_for_central_logs_bucket']
    public_access = s3_client.get_public_access_block(Bucket=bucket_name)
    block_config = public_access['PublicAccessBlockConfiguration']
...
def test_central_logs_bucket_restricts_public_buckets(
    s3_client: Any,
    config: Dict[str, Any]
) -> None:
    bucket_name = config['name_for_central_logs_bucket']
...
def test_central_logs_bucket_versioning_disabled(s3_client: Any, config: Dict[str, Any]) -> None:
    bucket_name = config['name_for_central_logs_bucket']
    versioning = s3_client.get_bucket_versioning(Bucket=bucket_name)
    assert versioning.get('Status') != 'Enabled'


def test_central_logs_bucket_has_log_delivery_write_acl(
    s3_client: Any,
    config: Dict[str, Any]
) -> None:
    bucket_name = config['name_for_central_logs_bucket']
...
def test_central_logs_bucket_ownership_is_bucket_owner_preferred(
    s3_client: Any,
    config: Dict[str, Any]
) -> None:
    bucket_name = config['name_for_central_logs_bucket']
...
def test_central_logs_bucket_has_lifecycle_configuration(
    s3_client: Any,
    config: Dict[str, Any]
) -> None:
    bucket_name = config['name_for_central_logs_bucket']
...
def test_central_logs_bucket_has_standard_ia_transition(
    s3_client: Any,
    config: Dict[str, Any]
) -> None:
    bucket_name = config['name_for_central_logs_bucket']
...
def test_central_logs_bucket_has_policy(s3_client: Any, config: Dict[str, Any]) -> None:
    bucket_name = config['name_for_central_logs_bucket']
    policy = s3_client.get_bucket_policy(Bucket=bucket_name)
    assert 'Policy' in policy


def test_central_logs_bucket_policy_denies_insecure_transport(
    s3_client: Any,
    config: Dict[str, Any]
) -> None:
    bucket_name = config['name_for_central_logs_bucket']
...
def test_central_logs_bucket_has_logging_enabled(s3_client: Any, config: Dict[str, Any]) -> None:
    bucket_name = config['name_for_central_logs_bucket']
    logging = s3_client.get_bucket_logging(Bucket=bucket_name)
    assert 'LoggingEnabled' in logging


def test_central_logs_bucket_logs_to_itself(s3_client: Any, config: Dict[str, Any]) -> None:
    bucket_name = config['name_for_central_logs_bucket']
    logging = s3_client.get_bucket_logging(Bucket=bucket_name)
...
def test_central_logs_bucket_policy_has_firehose_statement(
    s3_client: Any,
    config: Dict[str, Any]
) -> None:
    bucket_name = config['name_for_central_logs_bucket']
...
def test_central_logs_bucket_policy_firehose_allows_put_object(
    s3_client: Any,
    config: Dict[str, Any]
) -> None:
    bucket_name = config['name_for_central_logs_bucket']
...
def test_central_logs_bucket_policy_firehose_restricts_to_cloudwatch_logs_prefix(
    s3_client: Any, config: Dict[str, Any]
) -> None:
    bucket_name = config['name_for_central_logs_bucket']
    policy = s3_client.get_bucket_policy(Bucket=bucket_name)
    policy_doc = policy['Policy']
...
def test_central_logs_bucket_policy_firehose_requires_service_principal(
    s3_client: Any,
    config: Dict[str, Any]
) -> None:
    bucket_name = config['name_for_central_logs_bucket']
...
def test_central_logs_bucket_force_destroy_in_tfstate(central_logs_bucket_attrs: Any) -> None:
    assert central_logs_bucket_attrs['force_destroy'] is True


def test_terraform_state_bucket_force_destroy_in_tfstate(terraform_state_bucket_attrs: Any) -> None:
...
def test_cloudtrail_trail_is_multi_region(cloudtrail_client: Any) -> None:
    trails = cloudtrail_client.describe_trails()
    trail = trails['trailList'][0]
    assert trail['IsMultiRegionTrail'] is True


def test_cloudtrail_includes_global_service_events(cloudtrail_client: Any) -> None:
    trails = cloudtrail_client.describe_trails()
    trail = trails['trailList'][0]
    assert trail['IncludeGlobalServiceEvents'] is True


def test_cloudtrail_has_log_file_validation_enabled(cloudtrail_client: Any) -> None:
    trails = cloudtrail_client.describe_trails()
    trail = trails['trailList'][0]
    assert trail['LogFileValidationEnabled'] is True


def test_cloudtrail_is_actively_logging(cloudtrail_client: Any) -> None:
    trails = cloudtrail_client.describe_trails()
    trail = trails['trailList'][0]
...
def test_cloudtrail_s3_bucket_has_encryption(s3_client: Any, cloudtrail_client: Any) -> None:
    trails = cloudtrail_client.describe_trails()
    trail = trails['trailList'][0]
...
def test_cloudtrail_s3_bucket_versioning_disabled(s3_client: Any, cloudtrail_client: Any) -> None:
    trails = cloudtrail_client.describe_trails()
    trail = trails['trailList'][0]
    bucket_name = trail['S3BucketName']
    versioning = s3_client.get_bucket_versioning(Bucket=bucket_name)
...
def test_cloudtrail_has_cloudwatch_logs_configured(cloudtrail_client: Any) -> None:
    trails = cloudtrail_client.describe_trails()
    trail = trails['trailList'][0]
...
def test_access_log_bucket_has_encryption(s3_client: Any, access_log_bucket: Optional[str]) -> None:
    if access_log_bucket:
        encryption = s3_client.get_bucket_encryption(Bucket=access_log_bucket)
        assert 'ServerSideEncryptionConfiguration' in encryption


def test_access_log_bucket_versioning_disabled(
    s3_client: Any,
...
def test_access_log_bucket_has_standard_ia_transition_at_30_days(
    s3_client: Any,
    access_log_bucket: Optional[str]
) -> None:
...
def test_access_log_bucket_has_glacier_transition_at_90_days(
    s3_client: Any,
    access_log_bucket: Optional[str]
) -> None:
    lifecycle = s3_client.get_bucket_lifecycle_configuration(Bucket=access_log_bucket)
...
def test_cloudtrail_bucket_enforces_ssl(s3_client: Any, cloudtrail_client: Any) -> None:
    trails = cloudtrail_client.describe_trails()
    trail = trails['trailList'][0]
    bucket_name = trail['S3BucketName']
    policy = s3_client.get_bucket_policy(Bucket=bucket_name)
...
def test_hosted_zone_is_public(route53_client: Any, config: Dict[str, Any]) -> None:
...
def test_google_verification_txt_record_has_correct_value(
    txt_record: Any,
    config: Dict[str, Any]
) -> None:
...
def test_google_verification_txt_record_has_ttl(txt_record: Any) -> None:
    assert 'TTL' in txt_record


def test_gmail_mx_record_has_correct_priority(mx_record: Any) -> None:
    record_values = [rr['Value'] for rr in mx_record['ResourceRecords']]
    assert any('1 smtp.google.com' in val for val in record_values)


def test_gmail_mx_record_has_ttl(mx_record: Any) -> None:
    assert 'TTL' in mx_record


def test_txt_record_ttl_equals_300(txt_record: Any) -> None:
    assert txt_record['TTL'] == 300


def test_mx_record_ttl_equals_300(mx_record: Any) -> None:
    assert mx_record['TTL'] == 300


def test_mx_record_hostname_has_trailing_dot(mx_record: Any) -> None:
    record_values = [rr['Value'] for rr in mx_record['ResourceRecords']]
    assert any('smtp.google.com.' in val for val in record_values)


def test_mx_record_priority_equals_one(mx_record: Any) -> None:
    record_values = [rr['Value'] for rr in mx_record['ResourceRecords']]
    assert any(val.startswith('1 ') for val in record_values)


def test_iam_role_trust_policy_has_federated_principal(
    iam_client: Any,
    config: Dict[str, Any],
    aws_account_id: str
) -> None:
...
def test_iam_role_trust_policy_has_correct_audience_condition(
    iam_client: Any,
    config: Dict[str, Any]
) -> None:
    role_name = config['name_for_github_actions_role']
...
def test_iam_role_trust_policy_has_correct_subject_condition(
    iam_client: Any,
    config: Dict[str, Any]
) -> None:
    role_name = config['name_for_github_actions_role']
...
def test_iam_role_has_administrator_access_policy(iam_client: Any, config: Dict[str, Any]) -> None:
    role_name = config['name_for_github_actions_role']
...
def test_oidc_provider_has_correct_client_id(iam_client: Any, aws_account_id: str) -> None:
    account_id = aws_account_id
    provider_arn = f"arn:aws:iam::{account_id}:oidc-provider/token.actions.githubusercontent.com"
    response = iam_client.get_open_id_connect_provider(OpenIDConnectProviderArn=provider_arn)
...
def test_github_pat_parameter_type_is_secure_string(
    ssm_client: Any,
    config: Dict[str, Any]
) -> None:
...
def test_github_pat_parameter_has_value(ssm_client: Any, config: Dict[str, Any]) -> None:
...
def test_github_pat_parameter_value_is_not_placeholder(
    ssm_client: Any,
    config: Dict[str, Any]
) -> None:
    param_name = config['ssm_parameter_name_for_github_pat']
...
def test_github_app_id_parameter_type_is_string(ssm_client: Any, config: Dict[str, Any]) -> None:
...
def test_github_app_installation_id_parameter_type_is_string(
    ssm_client: Any,
    config: Dict[str, Any]
) -> None:
...
def test_github_app_private_key_parameter_type_is_secure_string(
    ssm_client: Any,
    config: Dict[str, Any]
) -> None:
...
def test_github_app_private_key_parameter_has_value(
    ssm_client: Any,
    config: Dict[str, Any]
) -> None:
    param_name = f"{config['github_app_ssm_prefix']}/private_key"
...
def test_terraform_state_bucket_encryption_is_aes256(
    s3_client: Any,
    config: Dict[str, Any]
) -> None:
    bucket_name = config['name_for_terraform_state_bucket']
...
def test_terraform_state_bucket_blocks_public_acls(s3_client: Any, config: Dict[str, Any]) -> None:
    bucket_name = config['name_for_terraform_state_bucket']
    public_access = s3_client.get_public_access_block(Bucket=bucket_name)
    block_config = public_access['PublicAccessBlockConfiguration']
    assert block_config['BlockPublicAcls'] is True
...
def test_terraform_state_bucket_blocks_public_policy(
    s3_client: Any,
    config: Dict[str, Any]
) -> None:
    bucket_name = config['name_for_terraform_state_bucket']
...
def test_terraform_state_bucket_ignores_public_acls(s3_client: Any, config: Dict[str, Any]) -> None:
    bucket_name = config['name_for_terraform_state_bucket']
    public_access = s3_client.get_public_access_block(Bucket=bucket_name)
    block_config = public_access['PublicAccessBlockConfiguration']
    assert block_config['IgnorePublicAcls'] is True
...
def test_terraform_state_bucket_restricts_public_buckets(
    s3_client: Any,
    config: Dict[str, Any]
) -> None:
    bucket_name = config['name_for_terraform_state_bucket']
...
def test_terraform_state_bucket_has_policy(s3_client: Any, config: Dict[str, Any]) -> None:
    bucket_name = config['name_for_terraform_state_bucket']
    policy = s3_client.get_bucket_policy(Bucket=bucket_name)
    assert 'Policy' in policy


def test_terraform_state_bucket_policy_denies_insecure_transport(
    s3_client: Any,
    config: Dict[str, Any]
) -> None:
    bucket_name = config['name_for_terraform_state_bucket']
...
def test_terraform_state_bucket_has_logging_enabled(s3_client: Any, config: Dict[str, Any]) -> None:
    bucket_name = config['name_for_terraform_state_bucket']
    logging = s3_client.get_bucket_logging(Bucket=bucket_name)
    assert 'LoggingEnabled' in logging


def test_terraform_state_bucket_logs_to_central_logs(
    s3_client: Any,
    config: Dict[str, Any]
) -> None:
    bucket_name = config['name_for_terraform_state_bucket']
