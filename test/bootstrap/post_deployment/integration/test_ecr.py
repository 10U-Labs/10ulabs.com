...
def test_ecr_repository_has_managed_by_tag(ecr_client, config):
    """Test that the repository has the ManagedBy tag."""
    repository_name = config["ecr_repository_name"]
    response = ecr_client.describe_repositories(repositoryNames=[repository_name])
...
def test_ecr_lifecycle_policy_has_agent_creator_rule(ecr_client, config):
...
def test_ecr_lifecycle_policy_has_workflow_fixer_rule(ecr_client, config):
...
def test_ecr_lifecycle_policy_has_test_auditor_rule(ecr_client, config):
