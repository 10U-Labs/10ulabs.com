...
def test_call_bedrock_with_retry_succeeds_on_first_attempt(mock_sleep):
    ...
    bedrock_config = {'model_id': 'test-model', 'max_tokens': 1000, 'budget_tokens': 5000}
...
def test_call_bedrock_with_retry_uses_correct_model_id():
    ...
    bedrock_config = {'model_id': 'custom-model-id', 'max_tokens': 1000, 'budget_tokens': 5000}
    messages = [{'role': 'user', 'content': [{'text': 'test'}]}]
    transform_module.call_bedrock_with_retry(mock_client, bedrock_config, messages)
...
def test_call_bedrock_with_retry_uses_correct_max_tokens():
    ...
    bedrock_config = {'model_id': 'test-model', 'max_tokens': 5000, 'budget_tokens': 5000}
    messages = [{'role': 'user', 'content': [{'text': 'test'}]}]
    transform_module.call_bedrock_with_retry(mock_client, bedrock_config, messages)
    mock_client.converse.assert_called_once()
    call_args = mock_client.converse.call_args
...
@patch('transform_to_compliant_markdown.time.sleep')
def test_call_bedrock_with_retry_includes_extended_thinking(mock_sleep):
    mock_client = Mock()
    mock_client.converse.return_value = {
        'output': {'message': {'content': [{'text': 'test'}]}}
    }
    ...
    transform_module.call_bedrock_with_retry(mock_client, bedrock_config, messages)
    call_args = mock_client.converse.call_args
    assert call_args[1]['additionalModelRequestFields'] == {'thinking': {'type': 'enabled', 'budget_tokens': 5000}}
