...
def test_returns_none_when_no_distribution_carries_the_domain() -> None:
    cloudfront = MagicMock()
...
def test_returns_the_id_of_the_distribution_carrying_the_domain() -> None:
    cloudfront = MagicMock()
...
def test_returns_the_first_id_when_two_distributions_carry_the_domain() -> None:
    cloudfront = MagicMock()
    cloudfront.list_distributions.return_value = _listing(
...
def test_returns_none_when_the_response_lists_no_distributions() -> None:
    cloudfront = MagicMock()
    cloudfront.list_distributions.return_value = {}
    assert find_distribution_id(cloudfront, "10ulabs.com") is None


def test_raises_runtime_error_when_status_never_completes() -> None:
    cloudfront = MagicMock()
...
def test_returns_none_when_the_first_status_read_says_completed() -> None:
    cloudfront = MagicMock()
    cloudfront.get_invalidation.return_value = {
        "Invalidation": {"Status": "Completed"}
    }
    wait_for_invalidation(cloudfront, "E1", "I1")
    assert cloudfront.get_invalidation.called


def test_sleeps_the_poll_interval_between_two_status_reads() -> None:
    cloudfront = MagicMock()
...
def test_stops_reading_once_the_status_reaches_completed() -> None:
    cloudfront = MagicMock()
    cloudfront.get_invalidation.side_effect = [
        {"Invalidation": {"Status": "InProgress"}},
        {"Invalidation": {"Status": "Completed"}},
...
def test_returns_1_when_no_distribution_carries_the_domain() -> None:
    cloudfront = MagicMock()
    cloudfront.list_distributions.return_value = _listing(
...
def test_returns_1_when_the_invalidation_never_completes() -> None:
    assert _run_main(_serving("InProgress"))[0] == 1


def test_returns_0_when_the_invalidation_completes() -> None:
    assert _run_main(_serving("Completed"))[0] == 0


def test_passes_each_comma_separated_path_as_its_own_stripped_item() -> None:
...
def test_builds_its_client_in_the_region_it_is_given() -> None:
