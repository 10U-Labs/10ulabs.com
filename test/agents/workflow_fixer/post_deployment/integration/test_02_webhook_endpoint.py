"""Integration tests for Workflow Fixer Webhook Endpoint.
...
class TestWebhookEventHandling:
    ...
    def test_03_ignores_workflow_fixer_workflow(self, webhook_url):
        """Verify webhook ignores its own workflow-fixer workflows."""
        if not webhook_url:
            pytest.skip("Webhook URL not configured")
        ...
        body = response.json()
        assert "message" in body
        assert "workflow-fixer" in body["message"].lower(), (
            f"Expected 'workflow-fixer' in response, got: {body}"
        )
