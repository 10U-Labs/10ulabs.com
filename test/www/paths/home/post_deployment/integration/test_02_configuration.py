...
def test_home_page_returns_html_content_type(website_url: str) -> None:
...
def test_home_page_uses_https(website_url: str) -> None:
    uses_https = website_url.startswith("https://")
    assert uses_https, f"Expected https://, got {website_url}"


def test_ads_txt_returns_text_content_type(website_url: str) -> None:
...
def test_ads_txt_contains_google_adsense_entry(website_url: str) -> None:
    response = requests.get(f"{website_url}/ads.txt", timeout=30)
...
def test_home_page_has_gtag_script(website_url: str) -> None:
    response = requests.get(website_url, timeout=30)
...
def test_home_page_has_gtag_config(website_url: str) -> None:
    response = requests.get(website_url, timeout=30)
...
def test_privacy_page_has_gtag_script(website_url: str) -> None:
...
def test_privacy_page_has_gtag_config(website_url: str) -> None:
    response = requests.get(f"{website_url}/privacy.html", timeout=30)
    gtag_config = "gtag('config', 'G-8YJFQC2EGV')"
    has_gtag_config = gtag_config in response.text
...
def test_spa_routing_returns_html_content_type(website_url: str) -> None:
...
def test_spa_routing_returns_non_empty_content(website_url: str) -> None:
