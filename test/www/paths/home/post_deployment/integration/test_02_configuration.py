...
class TestHomePageConfiguration:
    def test_home_page_returns_html_content_type(self, website_url: str) -> None:
        response = requests.get(website_url, timeout=30)
        content_type = response.headers.get("Content-Type", "")
        is_html = "text/html" in content_type
        assert is_html, f"Expected text/html, got {content_type}"

    def test_home_page_uses_https(self, website_url: str) -> None:
...
class TestStaticFilesConfiguration:
    def test_ads_txt_returns_text_content_type(self, website_url: str) -> None:
    ...
    def test_ads_txt_contains_google_adsense_entry(self, website_url: str) -> None:
        response = requests.get(f"{website_url}/ads.txt", timeout=30)
...
class TestGoogleAnalyticsConfiguration:
    def test_home_page_has_gtag_script(self, website_url: str) -> None:
        response = requests.get(website_url, timeout=30)
    ...
    def test_home_page_has_gtag_config(self, website_url: str) -> None:
        response = requests.get(website_url, timeout=30)
    ...
    def test_privacy_page_has_gtag_script(self, website_url: str) -> None:
    ...
    def test_privacy_page_has_gtag_config(self, website_url: str) -> None:
        response = requests.get(f"{website_url}/privacy.html", timeout=30)
        gtag_config = "gtag('config', 'G-8YJFQC2EGV')"
        has_gtag_config = gtag_config in response.text
...
class TestSPARoutingConfiguration:
    def test_spa_routing_returns_html_content_type(self, website_url: str) -> None:
    ...
    def test_spa_routing_returns_non_empty_content(self, website_url: str) -> None:
