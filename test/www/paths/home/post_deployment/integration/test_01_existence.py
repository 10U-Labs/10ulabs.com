...
def test_home_page_returns_200(website_url: str) -> None:
...
def test_home_page_has_content(website_url: str) -> None:
    response = requests.get(website_url, timeout=30)
    has_content = len(response.text) > 0
    assert has_content, "Home page returned empty content"


def test_robots_txt_exists(website_url: str) -> None:
...
def test_nonexistent_path_returns_200(website_url: str) -> None:
...
def test_deeply_nested_nonexistent_path_returns_200(website_url: str) -> None:
