...
def test_cloudfront_logging_enabled(logging_config: Any) -> None:
...
def test_cloudfront_logging_prefix_is_correct(logging_config: Any) -> None:
    prefix = logging_config.get("Prefix", "")
    assert prefix == "cloudfront-logs/website/"


def test_cloudfront_logging_excludes_cookies(logging_config: Any) -> None:
    assert logging_config.get("IncludeCookies", True) is False


def test_cloudfront_uses_https_redirect(default_cache_behavior: Any) -> None:
    policy = default_cache_behavior["ViewerProtocolPolicy"]
    assert policy == "redirect-to-https"


def test_cloudfront_compression_enabled(default_cache_behavior: Any) -> None:
...
def test_s3_bucket_blocks_public_policy(public_access_block: Any) -> None:
    assert public_access_block["BlockPublicPolicy"] is True


def test_s3_bucket_ignores_public_acls(public_access_block: Any) -> None:
    assert public_access_block["IgnorePublicAcls"] is True


def test_s3_bucket_restricts_public_buckets(public_access_block: Any) -> None:
    assert public_access_block["RestrictPublicBuckets"] is True


def test_s3_bucket_has_encryption(s3_client: Any, config: Dict[str, Any]) -> None:
    response = s3_client.get_bucket_encryption(Bucket=config["website_bucket_name"])
    rules = response["ServerSideEncryptionConfiguration"]["Rules"]
    assert len(rules) > 0


def test_s3_bucket_encryption_is_aes256(s3_client: Any, config: Dict[str, Any]) -> None:
    response = s3_client.get_bucket_encryption(Bucket=config["website_bucket_name"])
    rules = response["ServerSideEncryptionConfiguration"]["Rules"]
...
def test_s3_bucket_has_logging_enabled(s3_client: Any, config: Dict[str, Any]) -> None:
    response = s3_client.get_bucket_logging(Bucket=config["website_bucket_name"])
    assert "LoggingEnabled" in response


def test_s3_bucket_logging_target_is_central_logs(s3_client: Any, config: Dict[str, Any]) -> None:
    response = s3_client.get_bucket_logging(Bucket=config["website_bucket_name"])
