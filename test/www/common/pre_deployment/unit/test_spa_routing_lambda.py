...
def test_apex_returns_301_status() -> None:
    event = make_event(host="example.com", uri="/about")
    response = handler(event, None)
    assert response["status"] == "301"


def test_apex_returns_moved_permanently_description() -> None:
    event = make_event(host="example.com", uri="/about")
    response = handler(event, None)
    assert response["statusDescription"] == "Moved Permanently"


def test_apex_redirects_to_www_domain() -> None:
    event = make_event(host="example.com", uri="/about")
    response = handler(event, None)
...
def test_apex_redirect_preserves_path() -> None:
...
def test_nested_path_without_trailing_slash_returns_301() -> None:
    event = make_event(host="www.example.com", uri="/rack-designer/ABCD12345")
    response = handler(event, None)
    assert response["status"] == "301"


def test_nested_path_without_trailing_slash_redirect_location() -> None:
    event = make_event(host="www.example.com", uri="/rack-designer/ABCD12345")
    response = handler(event, None)
    location = response["headers"]["location"][0]["value"]
...
def test_path_without_extension_redirects() -> None:
    event = make_event(host="www.example.com", uri="/about")
    response = handler(event, None)
    assert response["status"] == "301"


def test_path_with_trailing_slash_gets_index_html() -> None:
    event = make_event(host="www.example.com", uri="/contact/")
    response = handler(event, None)
    assert response["uri"] == "/contact/index.html"


def test_nested_path_without_extension_redirects() -> None:
    event = make_event(host="www.example.com", uri="/rack-designer/config")
    response = handler(event, None)
    assert response["status"] == "301"


def test_deeply_nested_path_redirects() -> None:
    event = make_event(host="www.example.com", uri="/a/b/c/d")
    response = handler(event, None)
    assert response["status"] == "301"


def test_missing_host_header_returns_301() -> None:
    event = _event_without_host_header()
    response = handler(event, None)
    assert response["status"] == "301"


def test_missing_host_header_redirects_to_www() -> None:
    event = _event_without_host_header()
    response = handler(event, None)
    location = response["headers"]["location"][0]["value"]
...
def test_empty_host_header_returns_301() -> None:
    event = make_event(host="", uri="/about")
    response = handler(event, None)
    assert response["status"] == "301"


def test_empty_host_header_redirects_to_www() -> None:
    event = make_event(host="", uri="/about")
    response = handler(event, None)
    location = response["headers"]["location"][0]["value"]
    assert location == "https://www./about"
