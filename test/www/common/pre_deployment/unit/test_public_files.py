...
def test_ads_txt_file_exists() -> None:
    assert (PUBLIC_DIR / "ads.txt").exists()


def test_ads_txt_contains_google_adsense_entry() -> None:
    content = (PUBLIC_DIR / "ads.txt").read_text()
    assert ADS_TXT_CONTENT in content


def test_robots_txt_file_exists() -> None:
    assert (PUBLIC_DIR / "robots.txt").exists()


def test_privacy_html_file_exists() -> None:
