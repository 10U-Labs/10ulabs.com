...
def test_extracts_empty_block() -> None:
    content = "{}"
    result = _extract_block_content(content, 0)
    assert result == "{}"


def test_extracts_block_with_single_key_value() -> None:
    content = "{ key = value }"
    result = _extract_block_content(content, 0)
    assert result == "{ key = value }"


def test_extracts_block_with_whitespace() -> None:
    content = "{\n  key = value\n}"
    result = _extract_block_content(content, 0)
    assert result == "{\n  key = value\n}"


def test_extracts_block_from_middle_of_content() -> None:
    content = "prefix { inner } suffix"
    result = _extract_block_content(content, 7)
    assert result == "{ inner }"


def test_returns_remaining_content_when_unbalanced() -> None:
    content = "{ unclosed"
    result = _extract_block_content(content, 0)
    assert result == "{ unclosed"


def test_extracts_single_nested_block() -> None:
    content = "{ outer { inner } }"
    result = _extract_block_content(content, 0)
    assert result == "{ outer { inner } }"


def test_extracts_deeply_nested_blocks() -> None:
    content = "{ a { b { c } } }"
    result = _extract_block_content(content, 0)
    assert result == "{ a { b { c } } }"


def test_extracts_multiple_nested_blocks_at_same_level() -> None:
    content = "{ first { } second { } }"
    result = _extract_block_content(content, 0)
    assert result == "{ first { } second { } }"


def test_extracts_terraform_style_nested_blocks() -> None:
    content = """{
  environment {
    variables = {}
  }
...
def test_extracts_inner_block_when_starting_at_inner_position() -> None:
...
def test_returned_class_has_lifecycle_test_method(tmp_path: Path) -> None:
    TestClass = create_lambda_lifecycle_tests(tmp_path)
    assert hasattr(TestClass, 'test_lambda_with_env_vars_has_lifecycle_rule')


def test_returned_class_has_configured_test_method(tmp_path: Path) -> None:
    TestClass = create_lambda_lifecycle_tests(tmp_path)
    assert hasattr(TestClass, 'test_terraform_files_configured')


def test_returned_class_name_is_test_lambda_lifecycle(tmp_path: Path) -> None:
    TestClass = create_lambda_lifecycle_tests(tmp_path)
    assert TestClass.__name__ == 'TestLambdaLifecycle'


def test_uses_lambda_tf_as_default(tmp_path: Path) -> None:
    tf_file = tmp_path / "lambda.tf"
...
def test_default_creates_single_tf_path(tmp_path: Path) -> None:
    TestClass = create_lambda_lifecycle_tests(tmp_path)
    instance = TestClass()
    assert accepted(instance.test_terraform_files_configured)


def test_accepts_custom_tf_files_list(tmp_path: Path) -> None:
...
def test_accepts_multiple_tf_files(tmp_path: Path) -> None:
...
def test_lifecycle_test_passes_for_valid_lambda(tmp_path: Path) -> None:
    tf_file = tmp_path / "lambda.tf"
    tf_file.write_text(LAMBDA_WITH_LIFECYCLE_TF)
    TestClass = create_lambda_lifecycle_tests(tmp_path)
    instance = TestClass()
...
def test_lifecycle_test_fails_for_invalid_lambda(tmp_path: Path) -> None:
    tf_content = '''
resource "aws_lambda_function" "my_lambda" {
  function_name = "my-function"
  environment {
...
def test_lifecycle_test_skips_nonexistent_files(tmp_path: Path) -> None:
    TestClass = create_lambda_lifecycle_tests(tmp_path, tf_files=["nonexistent.tf"])
    instance = TestClass()
    assert accepted(instance.test_lambda_with_env_vars_has_lifecycle_rule)


def test_configured_test_passes_with_default_config(tmp_path: Path) -> None:
    TestClass = create_lambda_lifecycle_tests(tmp_path)
    instance = TestClass()
    assert accepted(instance.test_terraform_files_configured)


def test_configured_test_passes_with_custom_files(tmp_path: Path) -> None:
    TestClass = create_lambda_lifecycle_tests(tmp_path, tf_files=["a.tf", "b.tf"])
    instance = TestClass()
    assert accepted(instance.test_terraform_files_configured)


def test_checks_all_configured_tf_files(tmp_path: Path) -> None:
    good_tf = '''
