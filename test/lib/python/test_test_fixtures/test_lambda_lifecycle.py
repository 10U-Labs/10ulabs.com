...
class TestExtractBlockContentSimpleBlocks:
    def test_extracts_empty_block(self) -> None:
        content = "{}"
        result = _extract_block_content(content, 0)
        assert result == "{}"

    def test_extracts_block_with_single_key_value(self) -> None:
        content = "{ key = value }"
        result = _extract_block_content(content, 0)
        assert result == "{ key = value }"

    def test_extracts_block_with_whitespace(self) -> None:
        content = "{\n  key = value\n}"
        result = _extract_block_content(content, 0)
        assert result == "{\n  key = value\n}"

    def test_extracts_block_from_middle_of_content(self) -> None:
        content = "prefix { inner } suffix"
        result = _extract_block_content(content, 7)
        assert result == "{ inner }"

    def test_returns_remaining_content_when_unbalanced(self) -> None:
        content = "{ unclosed"
        result = _extract_block_content(content, 0)
...
class TestExtractBlockContentNestedBlocks:
    def test_extracts_single_nested_block(self) -> None:
        content = "{ outer { inner } }"
        result = _extract_block_content(content, 0)
        assert result == "{ outer { inner } }"

    def test_extracts_deeply_nested_blocks(self) -> None:
        content = "{ a { b { c } } }"
        result = _extract_block_content(content, 0)
        assert result == "{ a { b { c } } }"

    def test_extracts_multiple_nested_blocks_at_same_level(self) -> None:
        content = "{ first { } second { } }"
        result = _extract_block_content(content, 0)
        assert result == "{ first { } second { } }"

    def test_extracts_terraform_style_nested_blocks(self) -> None:
        content = """{
  environment {
    variables = {}
  }
  lifecycle {
    ...
    def test_extracts_inner_block_when_starting_at_inner_position(self) -> None:
...
class TestCheckLambdaLifecycleRulesPass:
    def test_passes_for_lambda_with_lifecycle_and_replace_triggered_by(
        self,
        tmp_path: Path
    ) -> None:
...
class TestCheckLambdaLifecycleRulesFail:
    def test_fails_for_lambda_with_env_vars_but_no_lifecycle(self, tmp_path: Path) -> None:
        tf_content = '''
...
'''
    ...
    def test_fails_for_lambda_with_lifecycle_but_no_replace_triggered_by(
        self,
        tmp_path: Path
    ) -> None:
        tf_content = '''
...
'''
    ...
    def test_error_message_contains_resource_name(self, tmp_path: Path) -> None:
        tf_content = '''
...
'''
    ...
    def test_error_message_mentions_kms_grants(self, tmp_path: Path) -> None:
        tf_content = '''
...
'''
    ...
    def test_fails_for_second_lambda_missing_lifecycle(self, tmp_path: Path) -> None:
        tf_content = '''
...
class TestCreateLambdaLifecycleTestsReturnType:
    def test_returns_class_object(self, tmp_path: Path) -> None:
        result = create_lambda_lifecycle_tests(tmp_path)
        assert isinstance(result, type)

    def test_returned_class_has_lifecycle_test_method(self, tmp_path: Path) -> None:
        TestClass = create_lambda_lifecycle_tests(tmp_path)
        assert hasattr(TestClass, 'test_lambda_with_env_vars_has_lifecycle_rule')

    def test_returned_class_has_configured_test_method(self, tmp_path: Path) -> None:
        TestClass = create_lambda_lifecycle_tests(tmp_path)
        assert hasattr(TestClass, 'test_terraform_files_configured')

    def test_returned_class_name_is_test_lambda_lifecycle(self, tmp_path: Path) -> None:
        TestClass = create_lambda_lifecycle_tests(tmp_path)
        assert TestClass.__name__ == 'TestLambdaLifecycle'


class TestCreateLambdaLifecycleTestsDefaultTfFiles:
    def test_uses_lambda_tf_as_default(self, tmp_path: Path) -> None:
        tf_file = tmp_path / "lambda.tf"
        tf_file.write_text('resource "aws_s3_bucket" "b" {}')
        TestClass = create_lambda_lifecycle_tests(tmp_path)
        instance = TestClass()
        assert accepted(instance.test_lambda_with_env_vars_has_lifecycle_rule)

    def test_default_creates_single_tf_path(self, tmp_path: Path) -> None:
        TestClass = create_lambda_lifecycle_tests(tmp_path)
        instance = TestClass()
        assert accepted(instance.test_terraform_files_configured)


class TestCreateLambdaLifecycleTestsCustomTfFiles:
    def test_accepts_custom_tf_files_list(self, tmp_path: Path) -> None:
...
class TestCreateLambdaLifecycleTestsTestMethodBehavior:
    def test_lifecycle_test_passes_for_valid_lambda(self, tmp_path: Path) -> None:
        tf_file = tmp_path / "lambda.tf"
        tf_file.write_text(LAMBDA_WITH_LIFECYCLE_TF)
        TestClass = create_lambda_lifecycle_tests(tmp_path)
        instance = TestClass()
