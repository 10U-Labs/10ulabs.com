...
def test_returns_magicmock() -> None:
    mock = create_mock_dynamodb_client("get_item")
    assert isinstance(mock, MagicMock)


def test_method_returns_default_empty_dict() -> None:
    mock = create_mock_dynamodb_client("get_item")
    result = mock.get_item()
    assert result == {}


def test_method_returns_custom_value() -> None:
...
def test_returns_none_explicit() -> None:
