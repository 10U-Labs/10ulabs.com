...
def _reply(status=200, payload=None, raises=False):
    response = MagicMock()
...
def test_endpoint_is_not_deployed_when_the_status_is_404():
...
def test_endpoint_is_not_deployed_when_the_status_is_500():
    with patch("test_fixtures.http_endpoint.requests.get") as get:
        get.return_value = _reply(status=500)
        assert not endpoint_is_deployed(BASE, PATH)


def test_endpoint_is_not_deployed_when_the_body_names_a_not_found_error():
    with patch("test_fixtures.http_endpoint.requests.get") as get:
        get.return_value = _reply(payload={"error": "Not Found"})
        assert not endpoint_is_deployed(BASE, PATH)


def test_endpoint_is_deployed_when_the_body_names_another_error():
    with patch("test_fixtures.http_endpoint.requests.get") as get:
        get.return_value = _reply(payload={"error": "Bad Request"})
        assert endpoint_is_deployed(BASE, PATH)


def test_endpoint_is_deployed_when_the_body_does_not_parse():
    with patch("test_fixtures.http_endpoint.requests.get") as get:
        get.return_value = _reply(raises=True)
        assert endpoint_is_deployed(BASE, PATH)


def test_endpoint_is_not_deployed_when_the_request_raises():
    with patch("test_fixtures.http_endpoint.requests.get") as get:
        get.side_effect = requests.exceptions.RequestException
        assert not endpoint_is_deployed(BASE, PATH)


def test_endpoint_is_asked_with_post_when_the_method_is_post():
    with patch("test_fixtures.http_endpoint.requests.post") as post:
...
def test_skip_if_endpoint_not_deployed_skips_when_it_is_not():
    with patch("test_fixtures.http_endpoint.requests.get") as get:
        get.return_value = _reply(status=404)
...
def test_skip_if_endpoint_not_deployed_returns_when_it_is():
    with patch("test_fixtures.http_endpoint.requests.get") as get:
