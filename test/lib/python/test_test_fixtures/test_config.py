...
def test_ignores_locals_block_declaration(tf_file: Callable[[str], Path]) -> None:
    result = parse_locals_file(tf_file('locals {\n  key = "value"\n}\n'))
    assert 'locals' not in result


def test_parse_locals_file_ignores_comment_lines(tf_file: Callable[[str], Path]) -> None:
    result = parse_locals_file(tf_file('# comment\n  key = "value"\n'))
    assert len(result) == 1


def test_resolves_module_shared_reference(tf_file: Callable[[str], Path]) -> None:
...
def test_returns_empty_for_unresolved_reference(tf_file: Callable[[str], Path]) -> None:
...
def test_ignores_module_reference_without_shared_config(tf_file: Callable[[str], Path]) -> None:
...
def test_constructs_github_repo(tf_file: Callable[[str], Path]) -> None:
    shared_config = {
...
def test_includes_resource_prefix_from_shared_config(tf_file: Callable[[str], Path]) -> None:
    shared_config = {
        'aws_region': 'us-east-1',
        'domain_name': 'example.com',
...
def test_includes_central_logs_bucket(tf_file: Callable[[str], Path]) -> None:
    shared_config = {
        'aws_region': 'us-east-1',
        'domain_name': 'example.com',
