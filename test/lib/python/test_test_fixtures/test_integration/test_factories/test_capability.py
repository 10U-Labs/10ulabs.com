...
def test_has_iam_capability_by_default() -> None:
    test_class = create_layer6_capability_tests()
    instance = test_class()
    capabilities = instance.get_enabled_capabilities()
...
def test_has_two_default_capabilities() -> None:
    test_class = create_layer6_capability_tests()
    instance = test_class()
    capabilities = instance.get_enabled_capabilities()
...
def test_has_test_capabilities_configured_method() -> None:
    test_class = create_layer6_capability_tests()
    assert hasattr(test_class, "test_capabilities_configured")


def test_has_test_can_list_lambda_functions_method() -> None:
    test_class = create_layer6_capability_tests()
    assert hasattr(test_class, "test_can_list_lambda_functions")


def test_has_test_can_list_iam_roles_method() -> None:
    test_class = create_layer6_capability_tests()
    assert hasattr(test_class, "test_can_list_iam_roles")


def test_create_layer6_capability_tests_lambda_capability() -> None:
    test_class = create_layer6_capability_tests(frozenset({"lambda"}))
    assert hasattr(test_class, "test_can_list_lambda_functions")


def test_create_layer6_capability_tests_lambda_does_not_raise_on_success() -> None:
    test_class = create_layer6_capability_tests(frozenset({"lambda"}))
    instance = test_class()
...
def test_create_layer6_capability_tests_lambda_fails_on_create_client_error() -> None:
    test_class = create_layer6_capability_tests(frozenset({"lambda"}))
    instance = test_class()
    mock_client = MagicMock()
...
def test_create_layer6_capability_tests_iam_capability() -> None:
    test_class = create_layer6_capability_tests(frozenset({"iam"}))
    assert hasattr(test_class, "test_can_list_iam_roles")


def test_create_layer6_capability_tests_iam_does_not_raise_on_success() -> None:
    test_class = create_layer6_capability_tests(frozenset({"iam"}))
    instance = test_class()
    mock_client = MagicMock()
...
def test_create_layer6_capability_tests_iam_fails_on_create_client_error() -> None:
    test_class = create_layer6_capability_tests(frozenset({"iam"}))
    instance = test_class()
    mock_client = MagicMock()
...
def test_has_ssm_test_when_ssm_in_capabilities() -> None:
    test_class = create_layer6_capability_tests(frozenset({"ssm"}))
    assert hasattr(test_class, "test_can_describe_ssm_parameters")


def test_does_not_have_ssm_test_when_ssm_not_in_capabilities() -> None:
    test_class = create_layer6_capability_tests(frozenset({"lambda"}))
    assert not hasattr(test_class, "test_can_describe_ssm_parameters")


def test_create_layer6_capability_tests_ssm_does_not_raise_on_success() -> None:
    test_class = create_layer6_capability_tests(frozenset({"ssm"}))
    instance = test_class()
    mock_client = MagicMock()
...
def test_create_layer6_capability_tests_ssm_fails_on_create_client_error() -> None:
    test_class = create_layer6_capability_tests(frozenset({"ssm"}))
    instance = test_class()
    mock_client = MagicMock()
...
def test_has_dynamodb_test_when_dynamodb_in_capabilities() -> None:
    test_class = create_layer6_capability_tests(frozenset({"dynamodb"}))
    assert hasattr(test_class, "test_can_list_dynamodb_tables")


def test_does_not_have_dynamodb_test_when_dynamodb_not_in_capabilities() -> None:
    test_class = create_layer6_capability_tests(frozenset({"lambda"}))
    assert not hasattr(test_class, "test_can_list_dynamodb_tables")


def test_create_layer6_capability_tests_dynamodb_does_not_raise_on_success() -> None:
    test_class = create_layer6_capability_tests(frozenset({"dynamodb"}))
    instance = test_class()
    mock_client = MagicMock()
...
def test_create_layer6_capability_tests_dynamodb_fails_on_create_client_error() -> None:
    test_class = create_layer6_capability_tests(frozenset({"dynamodb"}))
    instance = test_class()
    mock_client = MagicMock()
...
def test_has_logs_test_when_logs_in_capabilities() -> None:
    test_class = create_layer6_capability_tests(frozenset({"logs"}))
    assert hasattr(test_class, "test_can_list_log_groups")


def test_does_not_have_logs_test_when_logs_not_in_capabilities() -> None:
    test_class = create_layer6_capability_tests(frozenset({"lambda"}))
    assert not hasattr(test_class, "test_can_list_log_groups")


def test_create_layer6_capability_tests_logs_does_not_raise_on_success() -> None:
    test_class = create_layer6_capability_tests(frozenset({"logs"}))
    instance = test_class()
    mock_client = MagicMock()
...
def test_create_layer6_capability_tests_logs_fails_on_create_client_error() -> None:
    test_class = create_layer6_capability_tests(frozenset({"logs"}))
    instance = test_class()
    mock_client = MagicMock()
...
def test_has_s3_test_when_s3_in_capabilities() -> None:
    test_class = create_layer6_capability_tests(frozenset({"s3"}))
    assert hasattr(test_class, "test_can_list_s3_buckets")


def test_does_not_have_s3_test_when_s3_not_in_capabilities() -> None:
    test_class = create_layer6_capability_tests(frozenset({"lambda"}))
    assert not hasattr(test_class, "test_can_list_s3_buckets")


def test_create_layer6_capability_tests_s3_does_not_raise_on_success() -> None:
    test_class = create_layer6_capability_tests(frozenset({"s3"}))
    instance = test_class()
    mock_client = MagicMock()
...
def test_create_layer6_capability_tests_s3_fails_on_create_client_error() -> None:
    test_class = create_layer6_capability_tests(frozenset({"s3"}))
    instance = test_class()
    mock_client = MagicMock()
...
class TestCreateLayer6CapabilityTestsMultipleCapabilities:
    def _get_all_capabilities(self) -> Any:
        test_class = create_layer6_capability_tests(
    ...
    def test_has_lambda_when_all_specified(self) -> None:
        assert "lambda" in self._get_all_capabilities()

    def test_has_iam_when_all_specified(self) -> None:
        assert "iam" in self._get_all_capabilities()

    def test_has_ssm_when_all_specified(self) -> None:
        assert "ssm" in self._get_all_capabilities()

    def test_has_dynamodb_when_all_specified(self) -> None:
        assert "dynamodb" in self._get_all_capabilities()

    def test_has_logs_when_all_specified(self) -> None:
        assert "logs" in self._get_all_capabilities()

    def test_has_s3_when_all_specified(self) -> None:
        assert "s3" in self._get_all_capabilities()


def test_create_layer6_capability_tests_empty_capabilities() -> None:
