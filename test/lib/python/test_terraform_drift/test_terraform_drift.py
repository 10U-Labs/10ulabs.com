...
def test_log_group_exists() -> None:
    mock_client = MagicMock()
...
def test_bucket_not_found() -> None:
    mock_client = MagicMock()
...
def test_bucket_other_error() -> None:
    mock_client = MagicMock()
    mock_client.head_bucket.side_effect = ClientError(
...
def test_all_checkers_are_callable() -> None:
...
def test_returns_true_when_resource_exists(mock_boto3: MagicMock) -> None:
    mock_client = MagicMock()
    mock_client.get_function.return_value = {"Configuration": {}}
...
@patch("terraform_drift.boto3")
def test_creates_client_with_correct_service_and_region(mock_boto3: MagicMock) -> None:
    mock_client = MagicMock()
    mock_client.get_function.return_value = {"Configuration": {}}
    mock_boto3.client.return_value = mock_client
...
@patch("terraform_drift.boto3")
def test_calls_checker_with_correct_resource_name(mock_boto3: MagicMock) -> None:
    mock_client = MagicMock()
    mock_client.get_function.return_value = {"Configuration": {}}
    mock_boto3.client.return_value = mock_client
...
def test_lambda_function_name() -> None:
...
def test_parses_create_actions_returns_single_result(mock_run: MagicMock) -> None:
...
@patch("terraform_drift.subprocess.run")
def test_parses_create_actions_extracts_resource_type(mock_run: MagicMock) -> None:
    mock_run.return_value = MagicMock(stdout=_make_plan_output(), returncode=0)
    result = get_planned_creates(Path("/tmp/terraform"))
...
@patch("terraform_drift.subprocess.run")
def test_parses_create_actions_extracts_resource_name(mock_run: MagicMock) -> None:
    mock_run.return_value = MagicMock(stdout=_make_plan_output(), returncode=0)
    result = get_planned_creates(Path("/tmp/terraform"))
...
@patch("terraform_drift.subprocess.run")
def test_ignores_non_create_actions(mock_run: MagicMock) -> None:
...
@patch("terraform_drift.subprocess.run")
def test_ignores_unsupported_types(mock_run: MagicMock) -> None:
    mock_run.return_value = MagicMock(
...
@patch("terraform_drift.subprocess.run")
def test_handles_invalid_json(mock_run: MagicMock) -> None:
...
@patch("terraform_drift.subprocess.run")
def test_ignores_non_planned_change_type(mock_run: MagicMock) -> None:
...
@patch("terraform_drift.subprocess.run")
def test_ignores_refresh_type(mock_run: MagicMock) -> None:
...
@patch("terraform_drift.subprocess.run")
def test_ignores_create_with_empty_resource_name(mock_run: MagicMock) -> None:
...
@patch("terraform_drift.subprocess.run")
def test_ignores_create_with_missing_name_field(mock_run: MagicMock) -> None:
